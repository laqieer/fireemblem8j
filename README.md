...
| **Matching-C functions** | **~73.5%** (6,267 / 8,528) | functions whose bytes come from compiling `src/*.c`. The rest are **gbadisasm descriptive asm** (disassembly, *not* decompilation) — the ongoing asm→C grind. NB: the honest ceiling is **8,209**, not 8,528 — ~170 functions (ARM-mode `arm.o`/`m4a_1`, libgcc/libc) are hand-assembly even in the US decomp; against that ceiling matching-C is **~76.3%**. Key (D60/D63/D64/D67/D68): most of the remaining was mechanically reachable — the "~2,224 region-different" estimate was largely a classifier false-negative. Three levers cleared the bulk: the fast **reloc-resolve carve** (`scripts/perm2_graduate.py` — region-SAME modulo relocation), **CF:agbcc data-binding** (bind the TU-private data table a function references, then it compiles byte-exact), and **per-frag region-same harvest** (`scripts/perfrag_carve.py`). The true hand-decomp/permuter frontier (genuine FAR codegen + LEN/NOADDR) is materially smaller. |
| **Extracted data** | **~3.13%** (~437 KB) | genuinely-extracted asset bytes (typed C struct tables / PNG) ÷ data bytes. Named `.incbin "baserom.gba"` (of a committed `.bin`) is **not** counted as extraction. The big jump (D71) emits the ~404 KB JP message block as **compiled C** (`src/msg_data.c`: Huffman bitstreams + tree + `gMsgTable`, US-shaped) instead of a `.byte` asm blob — byte-identical after link; plus typed game-data tables (chapter/affinity/item/character/class/AI, D70–D76). NB the realistic data ceiling is ~50-70% (graphics LZ blobs are build-intermediate assets, not committed source — as in fe8u/pokeemerald), not 100%. |
