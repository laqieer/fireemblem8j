...
| **Named symbols** | **96.97%** (12,253 / 12,636) — DATA-residue-split waves named catch-all `data_` blobs into their fe8u-validated sub-objects (EventListScr/AnimScr/AnimSprite/REDA/UnitDefinition/EventScr), surpassing the earlier ~95.8% reachable-max estimate; `banim_`/`gfx_`/`snd_` labels remain fe8u's own auto-naming convention (un-named in fe8u itself) so the axis is still structurally < 100% — D307/W10 | labels with meaningful names ÷ total labels. The rest are the **383** `sub_/data_/nullsub_/sheet` placeholders. See [`docs/decisions.md`](docs/decisions.md) for the naming-lever history. |
