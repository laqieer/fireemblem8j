...
| **Matching-C functions** | **~77.2%** (6,577 / 8,528) | functions whose bytes come from compiling `src/*.c`. The rest are **gbadisasm descriptive asm** (disassembly, *not* decompilation) — the ongoing asm→C grind. NB: the honest ceiling is **8,209**, not 8,528 — ~170 functions (ARM-mode `arm.o`/`m4a_1`, libgcc/libc) are hand-assembly even in the US decomp; against that ceiling matching-C is **~80.1%**. Key (D60/D63/D64/D67/D68/D81): most of the remaining was mechanically reachable — the "~2,224 region-different" estimate was largely a classifier false-negative. Levers that cleared the bulk: the fast **reloc-resolve carve** (`scripts/perm2_graduate.py` — region-SAME modulo relocation), **CF:agbcc data-binding** (bind the TU-private data table a function references, then it compiles byte-exact), **per-frag region-same harvest** (`scripts/perfrag_carve.py`), and the **constant-diff carve** (`scripts/const_diff_carve.py` — D81: most of the "FAR" tail differs from US bytes ONLY in JP data-constants like msg-IDs `0x8A3`→`0x843`; substitute the JP literal and it compiles byte-exact). The true hand-decomp/permuter frontier (genuine FAR codegen + LEN/NOADDR) is materially smaller. |
...
| **Named symbols** | **~76.8%** | labels with meaningful names ÷ total labels. The rest are `sub_/data_/nullsub_/sheet` placeholders. |
