...
| **Shiftability** (data pointers) | **51.19%** (10,153 / 19,834) — **9,681** absolute pointers still hardcoded | fraction of pointers in extracted data expressed as **relocatable symbol references** vs baked-in absolute `0x08xxxxxx` addresses. Raw `INCBIN`/`​.byte` data dumps freeze pointers as constants — on any rebuild that shifts a section they dangle and the game crashes. The fix (D296): rewrite each residual `_ref` .c into a `__asm__` block of `.4byte Sym` relocations — byte-identical (`make compare` stays OK) yet fully shiftable. **0 dangling.** Retired 2,514 byte-exact (pure-pointer + all-EXACT + fe8u-verified dense tables, incl. all 20 chapter event groups). The remaining ~9,681 are struct-array/generic INTERIOR pointers gated by the **fe8u reloc oracle** (`scripts/fe8u_ptr_offsets.py`) — density alone is unsafe (D297). Audit with `scripts/audit_pointers.py`. |
