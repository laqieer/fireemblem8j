...
| **Named symbols** | **99.67%** (12,682 / 12,724) — DATA-residue-split waves named catch-all `data_` blobs into their fe8u-validated sub-objects (EventListScr/AnimScr/AnimSprite/REDA/UnitDefinition/EventScr); the metric now correctly counts the 136 descriptive asset labels `banim_pal_<key>` / `gfx_data_bg_<n>_<name>` as **named** (they identify the ROM asset, and fe8u's own `calcrom.pl` treats these identical labels as documented) — D307/W10, metric-calibration D-fix | labels with meaningful names ÷ total labels. The rest are the **42** genuinely-cryptic `sub_<hex>/data_<hex>/nullsub_` placeholders at the structural floor (multiboot image, padding, zero-pad tails, already-split blobs, region-diff MapChanges/EventList). See [`docs/decisions.md`](docs/decisions.md) for the naming-lever history (latest: **D355** residue-typing wave 5 FINAL, +3 AnimScr/Tsa relabels with 5 rigorous integrity skips; the 5-wave program typed 105 placeholders 98.84%→99.67% with 13 integrity skips and zero wrong names). |
