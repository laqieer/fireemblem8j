...
| **Matching-C functions** | **95.68%** (8,160 / 8,528) | functions whose bytes come from compiling `src/*.c` plus the handful linked from libc/libgcc archives. **~368 functions remain** — JP-only / JP-divergent code that cannot be ported verbatim from fe8u and needs real reconstruction. The winnable remainder is JP-divergent reconstruction (rebuild the JP function from its gbadisasm) + localized codegen levers + decomp-permuter; a large subset is a **proven agbcc compiler-version ceiling** (FE8-JP was built with an agbcc that none of ours/fe6j/fe7j reproduce — see [`docs/decisions.md`](docs/decisions.md) D275), so matching-C has a structural ceiling < 100% absent the original JP-FE8 toolchain. See [`docs/frontier.md`](docs/frontier.md). |
