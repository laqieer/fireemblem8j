...
| **Shiftability** (data pointers) | **38.51%** (7,639 / 19,834) — **12,195** absolute pointers still hardcoded | fraction of pointers in extracted data expressed as **relocatable symbol references** vs baked-in absolute `0x08xxxxxx` addresses. Raw `INCBIN`/`​.byte` data dumps freeze pointers as constants — on any rebuild that shifts a section they dangle and the game crashes. The fix (D296): rewrite each residual `_ref` .c from `INCBIN_U8` into `const u32 NAME[] = { (u32)&Sym, … }`; agbcc lowers `(u32)&sym` to a `.word` relocation, so the ROM is byte-identical (`make compare` stays OK) yet fully shiftable. **0 dangling** — every target already has a symbol, so the program is mechanical. Audit + retire with `scripts/audit_pointers.py`. |
