...
| **Matching-C functions** | **95.63%** (8,155 / 8,528) | functions whose bytes come from compiling `src/*.c` plus the handful linked from libc/libgcc archives. **~373 functions remain** — JP-only / JP-divergent code that cannot be ported verbatim from fe8u and needs real reconstruction (region-different *codegen*, cracked per-function with agbcc levers + decomp-permuter; a subset is an agbcc arg-order ceiling — see [`docs/decisions.md`](docs/decisions.md) D275). See [`docs/frontier.md`](docs/frontier.md) for the live remaining set and priority order. |
