...
| **Extracted data** | **~3.50%** (~488 KB) | genuinely-extracted asset bytes (typed C struct tables / PNG) ÷ data bytes. Named `.incbin "baserom.gba"` (of a committed `.bin`) is **not** counted as extraction. The big jump (D71) emits the ~404 KB JP message block as **compiled C** (`src/msg_data.c`: Huffman bitstreams + tree + `gMsgTable`, US-shaped) instead of a `.byte` asm blob — byte-identical after link; plus typed game-data tables (chapter/affinity/item/character/class/AI, D70–D76). NB the realistic data ceiling is ~50-70% (graphics LZ blobs are build-intermediate assets, not committed source — as in fe8u/pokeemerald), not 100%. |
