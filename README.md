...
| **Extracted data** | **~3.04%** (424,370 B) | genuinely-extracted asset bytes (typed C struct tables / PNG) ÷ data bytes. Named `.incbin "baserom.gba"` (of a committed `.bin`) is **not** counted as extraction. The latest jump (D70) emits the ~404 KB JP message block as **compiled C** (`src/msg_data.c`: the Huffman bitstreams + tree + `gMsgTable`, US-shaped) instead of a `.byte` asm blob — byte-identical after link. |
