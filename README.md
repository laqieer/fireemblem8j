...
| **Named symbols** | **99.51%** (12,662 / 12,724) — DATA-residue-split waves named catch-all `data_` blobs into their fe8u-validated sub-objects (EventListScr/AnimScr/AnimSprite/REDA/UnitDefinition/EventScr); the metric now correctly counts the 136 descriptive asset labels `banim_pal_<key>` / `gfx_data_bg_<n>_<name>` as **named** (they identify the ROM asset, and fe8u's own `calcrom.pl` treats these identical labels as documented) — D307/W10, metric-calibration D-fix | labels with meaningful names ÷ total labels. The rest are the **62** genuinely-cryptic `sub_<hex>/data_<hex>/nullsub_` placeholders. See [`docs/decisions.md`](docs/decisions.md) for the naming-lever history (latest: **D353** residue-typing wave 3, +26 `data_<hex>`→typed names — SjisGlyphs/gFontgrp 72-byte struct runs + gDebugMenuItems, byte-neutral, 3 MapChanges skipped for collision; D351/D352 were the +32/+27 earlier waves). |
