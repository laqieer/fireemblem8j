...
| **Shiftability** (data pointers) | **gate = 1** (essentially complete) — D307/W2 relocated +363 stuck `.4byte` literals (gate 364→1); the lone residual is `_RestoreBgm` (an even-Thumb-address that resolves via a code carve, not a data pointer). 14,746 relocated. | fraction of pointers in extracted data expressed as **relocatable symbol references** vs baked-in absolute `0x08xxxxxx` addresses. Raw `INCBIN`/`​.byte` data freezes pointers as constants — on any rebuild that shifts a section they dangle and the game crashes. The fix (D296/D297): rewrite each table into a `.4byte Sym` relocation block — byte-identical (`make compare` stays OK) yet shiftable. **The literal raw-`0x08xxxxxx` count (2,867) can never reach 0** — it is dominated by **2,567 coincidental constants** (graphics pixels, packed unit-stat bitfields where the level byte is 0x08, sine-table values, sound samples) that are NOT pointers and whose "relocation" would corrupt the shifted game. The honest metric (D304, cross-AI reviewed) is **confirmed-real + unclassified real-pointer debt**, computed by the fe8u reloc oracle (`scripts/fe8u_ptr_offsets.py`) plus positive-evidence structural classification (FUNC-interior / ASSET-interior / ROM-header / UnitDef-non-redas-field are provably not pointers). Driven 7,639 → 14,374 over the program; the final session cracked the hex-offset slice class (+97), the `impure_data` newlib `_reent` self-pointers (+3), and the **gFontgrp Shift-JIS `sjisNext` glyph linked-list (+234)**. Residual gate = 12: ~9 deep-blob coincidental, 1 LUT, 2 event-script operands (needs event-script-aware de-pointering). Audit with `scripts/audit_pointers.py --true-debt --gate`. |
| **Asset editability** | **~367 KB** opaque raw-incbin remaining (was 605 KB at the D307 CTO-session start) — D307 extracted 400 graphics blobs → editable PNG, 2001 AnimSpr OAM → ANIM_SPRITE macros (−165 KB), 30 event scripts → EVENT macros, + typed struct/array tables. Of the remainder: ~72 KB is fe8u-parity TSA/gfx binary floor + ~56 KB system/newlib + pointer-arrays (legitimately binary like fe8u); the real structured-logic target is now small (remaining event/proc scripts + a few `data_*`). | data that is game *logic* (event/proc scripts, struct tables, LUTs) should be typed/editable C, not opaque `u8[] = INCBIN` blobs. Graphics/tilemap `.bin` are exempt (binary like fe8u's `.4bpp`). Target: 0 opaque structured blobs. |
