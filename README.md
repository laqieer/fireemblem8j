...
| **Extracted data** | **~0.18%** (25,282 / 13,938,060 B) | genuinely-extracted asset bytes (typed C struct tables / PNG) ÷ data bytes. Named `.incbin "baserom.gba"` (of a committed `.bin`) is **not** counted as extraction. Latest (D70): the whole AI data TU `src/cp_data.c` (`struct AiScr` scripts, `gCpData_*`, `gAiCombatScoreCoefficientTable`, escape-point/item/heal tables, `gAi1/2ScriptTable`) ported as one relocatable typed-C object (+5,588 B), superseding ~25 misattributed named-incbin fragments. |
