...
| **Asset editability** | **~120 KB** opaque raw-incbin remaining (was 605 KB at the D307 start, ~550 KB at D309 start) — D309 removed 842 dead/orphan `.bin` (already typed-C or already-extracted, −318 KB), extracted 78 graphics/TSA blobs, and typed 155 `UnitDef_*` tables → `struct UnitDefinition[]` (−35.6 KB). The remainder is the **fe8u-parity binary floor**: ~86 KB region-different compressed graphics / index tables / pointer-arrays / zero-padding under `data_08XXXXXX` (fe8u also `*fill*`s or INCBINs these), ~12.5 KB m4a sound, ~9 KB compressed battle-anim. The structured-logic target is essentially cleared. | data that is game *logic* (event/proc scripts, struct tables, LUTs) should be typed/editable C, not opaque `u8[] = INCBIN` blobs. Graphics/tilemap `.bin` are exempt (binary like fe8u's `.4bpp`). Target: 0 opaque structured blobs. |
