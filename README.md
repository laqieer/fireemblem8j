...
| **Extracted data** | **~0.2%** | genuinely-extracted asset bytes (typed C struct tables / PNG) ÷ data bytes. Named `.incbin "baserom.gba"` (of a committed `.bin`) is **not** counted as extraction. D70 added `gItemData[206]` + its pointee tables (`data_items`/`data_itemuse`/`data_itembonus`, +7,940 B) — the largest single deferred game-data table — typed bottom-up so the pStatBonuses/pEffectiveness pointer columns link-resolve. |
