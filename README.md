...
| **Extracted data** | **98.52%** (of the measured set) | genuinely-extracted asset bytes (typed C struct tables / PNG / charmap text) ÷ measured data bytes. Named `.incbin "baserom.gba"` (of a committed `.bin`) is **not** counted as extraction. **Scope note (D311):** this counter measures C/PNG bytes only — the **72 `.mid` songs + 30 `.s` voicegroups** the music lane just made editable build from source via `mid2agb`/`as` but are **not** C/PNG, so they fall outside this counter even though they ARE from-source (captured by **Build self-containment = 100%**) and editable. So 98.52% here understates real extraction; the move was net-positive (more editable, still self-contained). Data is ~94% of the ROM by volume; see [`docs/frontier.md`](docs/frontier.md). |
