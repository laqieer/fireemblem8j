...
| **Matching-C functions** | **95.70%** (8,161 / 8,528) | functions whose bytes come from compiling `src/*.c` plus the handful linked from libc/libgcc archives. **~367 functions remain** — JP-only / JP-divergent code that cannot be ported verbatim from fe8u. The winnable remainder is JP-divergent reconstruction + localized codegen levers + decomp-permuter, **plus a newly-unlocked subclass**: the s8/s16 "hold-form" functions are matchable per-TU with `jp_agbcc` (a thumb-`PROMOTE_MODE`-patched agbcc that preserves sub-word type signedness — see [`docs/decisions.md`](docs/decisions.md) **D276**). Some remaining ceilings are still unattributed agbcc codegen divergences. See [`docs/frontier.md`](docs/frontier.md). |
