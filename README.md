...
| **Matching-C functions** | **~47.6%** (4,059 / 8,528) | functions whose bytes come from compiling `src/*.c`. The rest are **gbadisasm descriptive asm** (disassembly, *not* decompilation) — the ongoing asm→C grind. NB: the honest ceiling is **8,209**, not 8,528 — ~170 functions (ARM-mode `arm.o`/`m4a_1`, libgcc/libc) are hand-assembly even in the US decomp; against that ceiling matching-C is **~49.4%**. Remaining splits into ~region-same (verify-or-revert harvest lever) and region-different (m2c→permuter, validated — a pilot byte-matched 58, so the region-different count is lower than first estimated since many "region-different" functions are region-same once BL relocations resolve). |
