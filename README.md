...
| **Shiftability** (data pointers) | **80.44%** literal / **82.03%** real (11,670 relocatable) — **2,556** real hardcoded pointers left (+282 coincidental graphics constants) | fraction of pointers in extracted data expressed as **relocatable symbol references** vs baked-in absolute `0x08xxxxxx` addresses. Raw `INCBIN`/`​.byte` data freezes pointers as constants — on any rebuild that shifts a section they dangle and the game crashes. The fix (D296/D297): rewrite each table into a `__asm__` block of `.4byte Sym` relocations — byte-identical (`make compare` stays OK) yet shiftable. Driven from ~38% this session by the **fe8u reloc oracle** (`scripts/fe8u_ptr_offsets.py` + `repoint_table.py --fe8u-safe`): converts ONLY at fe8u-confirmed pointer byte-offsets, so a `struct UnitDefinition` data field coincidentally in ROM range stays raw (density alone is unsafe — D297). The remaining ~2,556 are un-named `data_*` blobs (need US-JP address mapping) + JP-divergent event scripts. Audit with `scripts/audit_pointers.py --metrics`. |
