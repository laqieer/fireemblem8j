...
| **Extracted data** | **~0.31%** | genuinely-extracted asset bytes (typed C struct tables / PNG) ÷ data bytes. Named `.incbin "baserom.gba"` (of a committed `.bin`) is **not** counted as extraction. Recently typed: `gCharacterData` (256×`struct CharacterData`) + `gClassData` (127×`struct ClassData`) — see `src/data/data_characters.c` / `data_classes.c`. |
