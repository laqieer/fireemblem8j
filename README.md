...
| **Extracted data** | **Strict C/PNG: 79.91%** (11,138,009 / 13,937,920); **source-form data: 99.26%** (13,835,356 / 13,937,920) | The strict badge/decomp.dev-compatible counter is deliberately narrow: bytes in `src/*.o` C/PNG/charmap/typed data plus libc/libgcc data ÷ measured data bytes. It is **not** gamed by crediting every non-`src/` object, so it stays 79.91%. The new supplemental **SOURCE-FORM DATA** line in `scripts/calcprogress.py` explains the apparent 20% path-filter gap: `banim/data_banim.o` (compressing-linker from `banim/*_motion.s` + `graphics/banim` PNG/AGBPAL), `sound/songs/midi/*.o` (mid2agb from `.mid`), `sound/voicegroups/*.o` (editable macro `.s`), `asm/fe6sio.o` (mgfembp-built FE6 payload), and the small m4a tables are all committed editable source with no `.incbin "baserom.gba"` fallback. The source-form line deliberately excludes the remaining **102,564 bytes** of opaque raw-incbin/`.bin` frontier. Named `.incbin "baserom.gba"` would still be residual and would not receive source-form credit; `scripts/check_selfcontained.py` currently proves there are 0 such directives in build-input source trees. |
