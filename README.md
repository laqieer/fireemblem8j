...
| **Matching-C functions** | **95.53%** (8,147 / 8,528) | functions whose bytes come from compiling `src/*.c` plus the handful linked from libc/libgcc archives. **~381 functions remain** — JP-only / JP-divergent code that cannot be ported verbatim from fe8u and needs real reconstruction. See [`docs/frontier.md`](docs/frontier.md) for the live remaining set and priority order, and [`docs/decisions.md`](docs/decisions.md) for the per-carve history. |
