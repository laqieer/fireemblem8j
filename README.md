...
| **Named symbols** | **99.98%** (12,721 / 12,723) — DATA-residue-split waves named catch-all `data_` blobs into their fe8u-validated sub-objects (EventListScr/AnimScr/AnimSprite/REDA/UnitDefinition/EventScr); the metric now correctly counts the 136 descriptive asset labels `banim_pal_<key>` / `gfx_data_bg_<n>_<name>` as **named** (they identify the ROM asset, and fe8u's own `calcrom.pl` treats these identical labels as documented) — D307/W10, metric-calibration D-fix | labels with meaningful names ÷ total labels. The rest are the **2** genuinely-cryptic `data_<hex>` placeholders — the D311 straddled-song `SongTrackData` baseline aliases (integrity-blocked: the proposed names mismatch the alias address + content, so they await a proper D311-anchor refactor). See [`docs/decisions.md`](docs/decisions.md) for the naming-lever history (latest: **D356** residue-typing wave 6 FINAL FLOOR, +39 structural-doc relabels + 1 stale-mirror delete with 2 integrity skips + 1 delete reverted after the no-baserom self-contained gate caught a false-OK; the 6-wave program typed 144 placeholders 98.84%→99.98% with 15 integrity skips and zero wrong names). |
