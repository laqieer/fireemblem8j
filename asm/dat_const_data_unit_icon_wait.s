	.section .rodata.dat_const_data_unit_icon_wait, "a", %progbits
	.align 2
@ Unit-icon WAIT map-sprite sheets (107 LZ77-compressed 4bpp sheets, one per class).
@ Region-SAME -- JP 0x081a71e0 == US 0x081b7828 byte-for-byte -- so the ROM bytes are
@ reproduced from EXTRACTED COMMITTED SOURCE, not baserom:
@   graphics/unit_icon/wait/*.png  --gbagfx-->  *.4bpp  --gbagfx -mindist N-->  *.4bpp.lz  --.incbin-->
@ -mindist is pinned PER ASSET in the Makefile (95 use the default 2; 12 need 1).
@ With baserom.gba REMOVED this region still builds byte-exact. Ported from
@ ../fireemblem8u by scripts/extract_graphics.py. See docs/tools/gbagfx.md.
