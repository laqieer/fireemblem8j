	.section .rodata.dat_data_item_icon, "a", %progbits
	.align 2
@ Item icon sheet: 224 item icons (16x16 4bpp, 0x80 bytes each) + a 32-color
@ palette (0x40). Region-SAME -- JP 0x085ba470 == US 0x085926f4 byte-for-byte -- so
@ the ROM bytes are reproduced from EXTRACTED COMMITTED SOURCE, not baserom:
@   graphics/item_icon/*.png  --gbagfx-->  *.4bpp  --.incbin-->  here
@   graphics/item_icon/item_icon_palette.agbpal  (committed binary)  --.incbin-->
@ With baserom.gba REMOVED this whole region still builds byte-exact. Ported from
