...
remaining diff is a small cluster: the narrow argument's shift pair is scheduled
too early, two stack slots are swapped, or a final stack reload lands in the wrong
register while the rest of the function is exact.

**Lever.** Preserve the caller's narrow prototype, but declare the callee-side
stacked parameter ABI-wide and narrow it at the semantic use point whose liveness
the target proves:
```c
/* caller still passes an s8 promoted to one ABI word */
int buildFlag = arg3;
int disableReda;

asm("" : "+m"(buildFlag));  /* only when JP proves this exact stack home */
disableReda = (s8)arg4;     /* delay narrowing until after that write */
```
Use the original argument directly when an extra lifetime alias changes its
extension order. If JP spills a narrow live value as a **word** around a call,
an `int iSpill`, scoped `+m`, and tied empty-asm reload can preserve the value
without adding a new narrowing pair:
```c
int iSpill = i;
asm("" : "+m"(iSpill));
value = RealCallee(realArg);
asm("" : "=r"(i) : "0"(iSpill));
```
A destination/readback carrier such as `register int buildRead asm("r5")` is
valid only when target disassembly proves that exact reload register. Do not add
stack homes, pins, fake calls, or readbacks speculatively: each one must correspond
to a concrete target live range and stack offset.

**Worked match and upstream provenance.** `GetUnitDefinitionFormEventScr`
(`sub_800FAD0`) uses direct `arg2`, `buildFlag@[sp,#0x40]`,
...
**When to stop checklisting and permute.** Rows 0–5 and 7 are the ones a quirk/flag/
source-cast or target-proven lifetime carrier fixes cheaply. Row 6 (pure scheduling) is the genuine
permuter/decomp-permuter case — but exhaust rows 0–5 and 7 first, because each is
