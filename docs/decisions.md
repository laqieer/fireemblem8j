...
## D252 — SioMenu_RestartGraphicsMaybe: coddog region-same NEAR (FID_ANNA+1) (+1 matching-C, 7889→7890)
`SioMenu_RestartGraphicsMaybe` (sub_8047CA0, 368B; fe8u sio_menu.c). coddog flagged it
region-same; the standard sub_ carve reached reloc-excluded diff=1, the single byte at +0x11e
being `movs r1,#100` (FID_ANNA) where JP wants 101 = `FID_ANNA + 1` (the JP face table inserts
one entry before ANNA — the SAME D203 fix). Plus extern the bound gSioMenu_0 (@0x080DEEB0) +
FaceConfig_SioMenu_1 (struct FaceVramEntry @0x085D3D70). diff 0.
LESSON: the coddog region_same_candidates leftovers aren't all dead — some are 1-byte-const
NEAR (decode the sadiff byte). 19 uncarved: 11 libc/libgcc (ceiling), 2 region-diff
(ply_memacc switch-lowering diff89, PidStatsAddSquaresMoved diff59), 5 FE8-specific not-found
in fe8u src (HandleCh5xUnits/RouteSplit/SetupAutoGeneratedUnitdefs — investigate), 1 carved.

## D253 — GetBanimTerrainGround: named-asm extern-inline helper (+1 matching-C, 7890→7891)
`GetBanimTerrainGround` (JP 0x08058BF4, 296B; fe8u banim-battleparse.c). The body is
`return _GetBanimTerrainGround(terrain, tileset) - 1` where `_GetBanimTerrainGround` is a
`static inline s8` helper (a switch over tileset returning BanimTerrainGround_Tileset01..NN
[terrain] — the tables are already bound). Provided the inline helper inline + the wrapper.
diff 0. Named-asm vein now down to libc/SRAM (SetSramFastFunc, VerifySramFast_Core, _Balloc,
malloc_extend_top, j_ClearOam veneer).

## D254 — prep_menuproc: synthetic sub_ → real fe8u names (source fidelity, byte-neutral)
nofuncmap_region_same.tsv has 1785 region-same name-mapped fns; filtering by NAME-not-in-src
found 44 "uncarved" — but these are FALSE POSITIVES: already matching-C, carved under SYNTHETIC
`sub_<addr>` names (the filter only checked the US name). So the nofuncmap_region_same vein is
EXHAUSTED for matching-C (all 1785 carved). The 3 reloc-unique (reliable 2-signal ID:
nofuncmap reloc-unique + byte-match diff0) were carved as synthetic reconstructions
(src/sub_80992B4/D0/EC.c using cw_8002DEC/raw-addr); replaced with the real fe8u source
(SetPrepScreenMenuOnBPress/OnStartPress/OnEnd from prep_menuproc.c: Proc_Find(ProcScr_PrepMenu)
...
**Result: 12/16 `PROVEN-BOUNDED`** (sub_8001570/800A594/800E1FC/807C8DC/807D3BC/
80A2E64/80A3300/80A3528/80A390C/80A6D34/80A6E4C/80C05C8; highest proving unroll
depth reported). 2 `DIVERGENCE` (sub_800A34C = stack-frame local buffers at
different spill offsets passed by pointer to a callee; sub_80A6F1C = indirect
callback with a stack address escaping to it — NOT confirmed reconstruction bugs).
2 `UNKNOWN:path-explosion` (sub_800FAD0 = 5 loops, reduces to DIVERGENCE with a
larger budget; sub_8057F80 = 1248 insns/149 br/58 calls). Sound fixes drove 5→12:
(a) target vs candidate sizes differ — use the true target size from assembling
`asm/*.s`; (b) read a function's own relocated literal pool from its OWN bytes, not
the shared cartridge ROM; (c) report the highest loop-unroll depth that proves;
(d) a **sound interprocedural callee-arg-liveness** (read-before-write over each
callee's `(addr, written-set)` states, recursing into `bl` targets; conservative
on `svc`/`bx rN`/unknown/cycles) compares only the args a callee actually reads
(e.g. `Decompress`={r0,r1}, `EventStartFade`={}); (e) compare call targets by
address (direct `bl F` == indirect `bx &F`); (f) de-truncate relocation symbol
names (`readelf -W`). The prover is **sound but incomplete**: PROVEN really is
equivalent (under the ARM/ABI/memory/call model, loops to depth N); the remaining
4 need relational stack-frame modelling / whole-program context / state-merging.
...
printed 12 suspects, but **D378 corrects that to 11 actual-range candidates**: it wrongly stretched
...
- **Initially mis-called "REAL baked frame-pointers" (3) — CORRECTED same-session to OAM data,
  ...
  opaque review set when typed. After that correction and D378's removal of the false popup
...
- `scan_opaque_selfref_suspects()` — the Finding-2 scanner; originally reported review suspects
  without gating them. **D378 replaces that perpetual review bucket** with exact-size scanning,
...
self-refs were still only surfaced). **D378 supersedes that gate behavior:** unresolved self-refs
...
verified win; D378 replaces `scan_opaque_selfref_suspects()` with exact-size candidate scanning
...
## D369 — combine five raw-score harvests without semantic or shiftcheck regressions; matching-C 99.92% (8685/8692), 7 remain (2026-07-11)
...
## D377 — lexically gate code-side raw ROM literal pools (issue #143 lane) (2026-07-13)
...
## D378 — hard-gate opaque self-reference evidence; correct the zero-size extent false positive (2026-07-13)
...
`AnimSprite_*` OAM/AnimScr-family labels** and 209 others
**[UPDATED by D380, 2026-07-14: fixing `data_08B5B560`'s one false relocation
makes that provider fully relocation-free, legitimately promoting it into this
zero-size population — 1,075 total / 210 other, 865 `AnimSprite_*` unchanged.
See D380's integration note.]**. No other
...
baseline (updated by D380 to 1,075/210 other after fixing `data_08B5B560`'s
false relocation — see D380); and a synthetic unknown self-ref becoming
...
## D379 — issue143 oversized-TSA/multi-resource outlier migrations (2026-07-14)

(Originally drafted as D378; renumbered at integration to D379 because the
concurrent D378 opaque-self-reference lane above landed first on `main`, per
this entry's own "renumber if a concurrent sibling claimed D378" note.)
...
`scripts/audit_bin_forms.py`: `.bin` net **+3** (-1 deleted monolith, +4 new
raw-TSA/tilemap FLOOR sources -- `Tsa_LinkArenaTitleBanner.bin`,
...
rather than editing the shared classifier script), `UNCERTAIN` unchanged (this
lane touches only FLOOR/TOTAL). On this branch's own `ab44df71e` base that was
**FLOOR 1415→1418 / TOTAL 1445→1448 / UNCERTAIN 30**; re-verified at
integration onto `main` (after the D376 map and D378 self-ref lanes) as
**FLOOR 1417→1420 / TOTAL 1445→1448 / UNCERTAIN 28** (unchanged) -- same net
+3/+3/+0 delta, different absolute baseline. Strict editable-source gain:
Recipe C = 32396 decoded bytes (31744 image
...
## D380 — reject proven false ABS32 decodes: function interiors and semantic domains (2026-07-13; header-domain completion 2026-07-14)
...
**ROM-header-domain completion.** The independent header verifier classified all
12 ROM-site relocations resolving inside `[0x08000000,0x080000C0)` as WRONG-ID,
with zero unresolved. The eight TileAnimations3 scalar pairs above were the
first subset. Four more remained:

- `Tsa_OpAnimEphraimClose2+0xB4` at `0x08B5B614` is bytes inside a valid LZ77
  stream (`10 00 05 00`, decoded size `0x500`), not a pointer to `Init`.
  JP and FE8U decompressed bytes are identical
  (`sha256=3c21ab5db845eb628cd69d2cea20187cee91697092472d5f798920b1db9c1269`);
  hypothetically relocating the word to `0x08040000` makes the stream malformed.
- `0x08FE0234`, `0x08FE14D8`, and `0x08FE200C` are fields in a 16 KiB
  DACS/IS-AGB debug-monitor image with its own embedded address domain. They are
  respectively host/debug metadata, a saved-PC initializer, and a cartridge-ROM
  base field, not parent-ROM pointers. Emit raw `0x08000000`. Keep the legacy
  public name `MultiBootImage_08FE0000` to avoid consumer churn, but correct its
  documented identity: it is not a multiboot/download-play child.
...
For the header class, inventory every ROM-site relocation whose final linked
word is in `[0x08000000,0x080000C0)`, regardless of symbol name. Proven source
schemas override all target typing: the eight TileAnimations3 u16 scalar slots,
the full `Tsa_OpAnimEphraimClose2` LZ range, and the 16 KiB DACS image range are
always HIGH. Outside those domains, accept only an exact `STT_FUNC` entry, an
exact nonzero-sized `STT_OBJECT` start, or an explicitly audited pinned slot;
untyped/unaudited header targets fail closed. Regression fixtures cover all
12 wrong slots, the TileAnimations3 record `+4` frame pointers, genuine typed
header code/object pointers, pinned cases, and the intentional raw-domain
exceptions needed to avoid misclassifying the corrected constants as missed
pointers.
...
relocations, the three compressed-stream records, and the four remaining
header-domain failures, the final count is **59,485**; FUNC counts are
unchanged. Exactly **12/12** header-resolving false relocations are gone and the
final ELF reports zero header-domain relocations. The scalar gate reports zero
large-addend, compressed-anchor, or AREA failures and zero placement-anchor
reviews. All twenty-two focused tests pass. `make compare` is OK and
`make shiftcheck` passes.
...
header and remain unchanged). This decision fixes and gates these proven
reopened issue-#143 false-positive classes only; it does not declare the issue
fully closed.

**Integration note (D378 cross-lane interaction).** Fixing `data_08B5B560`
(`Tsa_OpAnimEphraimClose2`)'s one false `Init` relocation above makes that
provider's object fully relocation-free, which legitimately promotes it into
D378's structureless-opaque scan population. Since the symbol carries no ELF
size, it lands in D378's zero-size `skipped` bucket rather than being scanned:
the "other" (non-`AnimSprite_`) zero-size count moves from 209 to **210** and
the total from 1,074 to **1,075**; the 865 `AnimSprite_*` count is unchanged.
D378's own regression test and documented figures are updated to this new,
correct baseline as part of landing this decision; this is a real, intended
side effect of removing a proven false relocation, not drift or an error.
Also note total linked ROM ABS32 grows from this decision's own 59,485 to
...
## D383 — false-zero shiftability gate: 10 unrelocated `TextGlyphs_System` links (CP932 97 CE 緑 fallback), a four-C-initializer blind spot, and a new structural glyph-relocation gate (2026-07-13)
...
## D384 — second false-zero shiftability class: 3 unrelocated `PROC_NAME` pointers in `ProcCmd` scripts, a structural gate for the whole consumer class, and a scan_offsets.py false-positive fix surfaced along the way (2026-07-13)

**The bug (issue #143, same-scope follow-up to D383).** `struct ProcCmd { short
...
original address. Same root class as D383 (a second consumer of the identical
...
**Scope note.** Same as D383: a targeted class-wide closure across a second
consumer, not a new numeric scorecard axis. `docs/frontier.md` axis-5 already
records the D383 closure; no additional edit needed there since the coarse
