...
## D377 — migrate six ending-frontier raw blobs; expand worldmap_minimap_p0 / worldmap_skirmish (2026-07-13)

_Renumberable: concurrent issue #143 branches may independently claim D377; the
integrator should renumber on merge if another D377 already landed on main._
...
## D377 follow-up — complete the gWorldmapMinimap_8..13 remainder; correct comment/audit-delta errors (2026-07-14)
...
The +14 FLOOR are all `.tsa.bin` sources (14 files: the 3 from blob 007, 1
...
`gWorldmapMinimap_12/13`); the palettes (`.pal`/JASC) and images (`.png`) are
...
output. `docs/frontier.md`'s session note and this file's earlier `D377` text
...
**Renumbering note.** This decision is filed locally as `D377`
(follow-up appended in place, no new number). Per the previous review,
`main` may already have advanced past `D376`/`D377`/`D378` and be
integrating `D379` by the time this branch lands — the integrator must
renumber this entry to the actual next-free slot at merge time.
