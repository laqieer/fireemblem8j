...
## D378 — D377 was INCOMPLETE: source-declaration enumeration missed 3 more disguised `u32[]` ProcCmd scripts; broadened the gate to a byte-level strict decode of every ROM object symbol (2026-07-13)

**D377 undercounted.** Its gate enumerated only symbols the C source itself
...
`ProcScr_efxThunderBG`/`BGCOL`/`efxFireOBJ` `PROC_NAME` literals D377 fixed,
PLUS three more in the same file that D377's source-declaration scan could
...
C-type-vs-actual-schema mismatch as D376's glyph `u32[]` blind spot and the
`sCpProcData`-style mistyped-extern class D377 itself already documented as a
...
elegantly also retires D377's earlier "must restrict to GLOBAL symbols to
...
against a scratch rebuild of the pre-D378 source reproduces exactly the
...
**Lesson (generalizes D376 + D377).** A source-language type scan — whether
...
class now: `u32[]` hiding a `struct Glyph.sjisNext` (D376), an `extern struct
ProcCmd` forward-declaring a symbol whose real definition is a `u32[]`
elsewhere (D377's `sCpProcData`), and now a `u32[]` DEFINITION hiding real
`struct ProcCmd` data consumed via an explicit cast in a different TU (D378).
...
**Scope note.** Same as D376/D377: a targeted class-wide closure (a third
...
the D376/D377 closures and its gate-count-stays-0 framing is unchanged by this
broadening.
...
## D381 — D380's zero-size backstop reintroduced the exact cross-object annexation bug through a different path; add a hard nonzero-size boundary invariant + a general multi-object-crossing audit, and make the relocation-completeness/shifted-A/B denominators physical-address-truthful (2026-07-14)

**The defect.** D380's zero-size backstop reused `extended_bound()` -- the
...
happened to decode validly (the exact cross-object bug D379 already fixed
...
11,110 (+1 vs D380 -- `ShopFadeIn` moved back); zero-size (recovered): 363
(-1 vs D380's 364); script prefixes: 994 (-1); records: 7,772 (non-unique);
...
Tracked on the same issue #143 thread as D376/D377/D378/D379/D380. Same
numbering-collision note applies: D381 (this entry) is ALSO a locally-scoped
number and must be renumbered by the integrator alongside D376-D380 against
`main`'s actual next-available sequence at merge time.
