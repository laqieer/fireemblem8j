...
| --- | --- | --- |
| byte-matching | in `make compare` (SHA-1) | 10 of the original 16 had graduated when this table was written. `DecodeLinkArenaRecordHeader`, `GetUnitDefinitionFormEventScr`, `GmapScreen2_Loop`, and `SplineEvalCatmullRom` later graduated; the live frontier is now **2** — see the cohort-status note near the top of this document |
