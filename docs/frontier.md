...
> [Superseded — see CURRENT STATE 2026-07-03 below] Historical snapshot; **two** axis lines here are stale: **(1)** MATCHING-C 99.64% / 31 still descriptive asm (live: **99.86% / 12 unmatched**), and **(2)** the SHIFTABILITY (axis #5) gate = 0 ✅ COMPLETE (D309) line just below (live: **gate = 1 (floor)** — the fe8u-confirmed coincidental constant `0x080896ED`). The live figures are in the authoritative 2026-07-03 six-axis table near the end of this doc.
...
### Six-axis scorecard refresh (axis-2 refreshed 2026-07-10; axis-3/6 refreshed 2026-07-09; verbatim source = `python3 scripts/calcprogress.py`)
| # | Axis | Value | Detail |
|---|---|---|---|
...
| 2 | Matching-C | **99.86% (8680/8692)** | 8553 `src/*.c` + 127 libc/libgcc real source; **12 still descriptive asm**. Axis-2 moved **22 → 12** through 10 byte-exact matches; this release candidate adds `DivinationRankSpriteUpdate` via score-0 `l4bts` (D367). |
...
> **Code-byte companion (frogress/decomp.dev `code` measure, source-form):** **99.47%** (896,628 / 901,428 bytes); **0.53% = 4,800 bytes** remains descriptive asm. Verbatim from `python3 scripts/calcprogress.py`; see D343/D367.

### Code frontier — the 12 remaining functions (axis-2 = 22 → 12; 10 matches banked)

> **2026-07-11 release candidate (D367):** `DivinationRankSpriteUpdate`
> (`sub_80A2E64`, 436 B) now compiles byte-exact from score-0 community fork
> `l4bts`. The source reuses one `data` pointer for the sine-table reads, commits
> the next IV before the branch split, pins the affine results to r6/r5/r4, and
> uses two empty `+r` fences after `Div` under `-fno-gcse`. This preserves JP's
> proc/table/result live ranges without raw opcode asm; decomp.me `.set`
> scaffolding was removed. Owned `qksQG` was updated from `l4bts`, verified raw
> score 0, then retired from the registry. Main CI remains the publication gate.
...
**Authoritative set:** `find src/nonmatching -maxdepth 1 -name '*.c'` returns **12**.
...
3. **sub_800E1FC** — Event18_ColorFade; `-mjp-promote`; mnemonic-identical spill/reg-allocation residual. — decomp.me: https://decomp.me/scratch/nlJVc
4. **sub_800FAD0** — GetUnitDefinitionFormEventScr; `-mjp-promote`; clean register-permutation NEAR. — decomp.me: https://decomp.me/scratch/eZzgG
5. **sub_807C8DC** — AdjustNewUnitPosition; `-mjp-promote`; branch coloring + loop-counter rotation. — decomp.me: https://decomp.me/scratch/taZrH
6. **sub_807D3BC** — SelectSummonPos; `-mjp-promote`; extra spill-slot/frame-layout decision. — decomp.me: https://decomp.me/scratch/J1ka1
7. **sub_80A3300** — augury rank-sprite emit; inner-loop coloring/spill tie-break. — decomp.me: https://decomp.me/scratch/ENay1
8. **sub_80A3528** — DrawAuguryResultPanel; invariant scheduling + register coloring. — decomp.me: https://decomp.me/scratch/MaiDT
9. **sub_80A6D34** — link-arena header decode; pure register coloring. — decomp.me: https://decomp.me/scratch/9myLi
10. **sub_80A6E4C** — link-arena record encode; spill / callback-veneer register choice. — decomp.me: https://decomp.me/scratch/g7FXU
11. **sub_80A6F1C** — DecodeAndVerifyArenaRecord; callback-in-high-register veneer residual. — decomp.me: https://decomp.me/scratch/h2W8F
12. **sub_80C05C8** — GmapScreen2_Loop; clean JP-vs-US coloring divergence. — decomp.me: https://decomp.me/scratch/R7AaX

**Play-Ranking / password-codec subsystem = sub_80A3300 / sub_80A3528 /
sub_80A6D34 / sub_80A6E4C / sub_80A6F1C (5 of the 12).**
