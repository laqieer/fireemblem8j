...
| 2 | Matching-C | **99.93% (8686/8692)** | 8559 `src/*.c` + 127 libc/libgcc real source; **6 still descriptive asm**. The five-function D369 harvest plus `DecodeAndVerifyArenaRecord` move the 22-function cohort to **6**. |
...
> **Code-byte companion (frogress/decomp.dev `code` measure, source-form):** **99.69%** (898,664 / 901,428 bytes); **0.31% = 2,764 bytes** remains descriptive asm. Verbatim from `python3 scripts/calcprogress.py`; see D343/D369.

### Code frontier — the 6 remaining functions (axis-2 = 22 → 6; 16 matches banked/candidate)

> **2026-07-11 DecodeAndVerify follow-up candidate:**
> `DecodeAndVerifyArenaRecord` (`sub_80A6F1C`, 212 B) now compiles byte-exact from
> source on `feat/axis2-compute-80a6f1c-copilot` (`6aa5f35fa`). The match uses
...
**Password-codec remainder = sub_80A6D34 only (1 of the 6).**
The Play-Ranking draw/sprite pair and both password entry points are now matching C.
