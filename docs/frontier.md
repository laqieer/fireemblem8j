...
- **SHIFTABILITY (axis #5, D296–D304): relocated 14,374 (from 7,639, +6,735); fe8u-confirmed real-pointer debt = 0; COMPLETION GATE = 12.** ESSENTIALLY COMPLETE — every confirmed-real, mechanically-convertible data pointer is relocated. The literal raw-`0x08xxxxxx` count (2,867) is the WRONG invariant for completion: it is dominated by **coincidental constants** (2,567 — graphics pixels, packed unit-stat bitfields where byte3=0x08, sine-table values, sound samples) that are NOT pointers and CANNOT be relocated (doing so corrupts the shifted game). Authoritative metric: `scripts/audit_pointers.py --true-debt --gate` (fe8u oracle + positive-evidence structural classification), NOT the literal raw count. **Gate = confirmed-real + unclassified DATA-pointer debt = 12**: ~9 deep-blob coincidental (sparse words in 18-21KB `gUnkData_*` pointing 60-130KB into other blobs, conservatively left unclassified), 1 `lut.29`, **2 `EventScr_Ch16A_1` event-script operands** (the only genuinely-real residual → needs event-script-aware de-pointering, a scoped subsystem task; a coincidental bytecode word mis-converted would corrupt shift, so NOT force-converted). `gap_000B1030`'s "pointers" are a Thumb function's literal pool → **code-decompilation axis**, not data debt. D304 fixed 5 real converter/auditor bugs (hex-offset slice regex, per-word gate convergence, slice-base address, auditor `incbin_ranges` hex+ERE, `RESID` crash) and added 2 conversion wins (`impure_data` newlib `_reent` self-pointers +3; **gFontgrp Shift-JIS `sjisNext` glyph linked-list +234** via `--glyphs`).
