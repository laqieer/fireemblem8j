...
> [Superseded — see CURRENT STATE 2026-07-03 below] Historical snapshot; **two** axis lines here are stale: **(1)** MATCHING-C 99.64% / 31 still descriptive asm (live: **99.85% / 13 unmatched**), and **(2)** the SHIFTABILITY (axis #5) gate = 0 ✅ COMPLETE (D309) line just below (live: **gate = 1 (floor)** — the fe8u-confirmed coincidental constant `0x080896ED`). The live figures are in the authoritative 2026-07-03 six-axis table near the end of this doc.
...
| 2 | Matching-C | **99.85% (8679/8692)** | 8552 `src/*.c` + 127 libc/libgcc real source; **13 still descriptive asm**. Axis-2 moved **22 → 13** through 9 byte-exact matches; this release candidate adds `AddAttr2dBitMap` and `Augury_InitResultScreen` (D366). |
...
> **Code-byte companion (frogress/decomp.dev `code` measure, source-form):** **99.42%** (896,192 / 901,428 bytes) = **96.44%** compiled from `src/` + **2.57%** linked from libc/libgcc; **0.58% = 5,236 bytes** remains descriptive asm. Verbatim from `python3 scripts/calcprogress.py`; see D343/D366.

### Code frontier — the 13 remaining functions (axis-2 = 22 → 13; 9 matches banked)
...
**Authoritative set:** `find src/nonmatching -maxdepth 1 -name '*.c'` returns **13**.
...
7. **sub_80A2E64** — DivinationRankSpriteUpdate; `-O2`; reg-allocation / IV-scheduling tie-break. — decomp.me: https://decomp.me/scratch/qksQG
8. **sub_80A3300** — augury rank-sprite emit; inner-loop coloring/spill tie-break. — decomp.me: https://decomp.me/scratch/ENay1
9. **sub_80A3528** — DrawAuguryResultPanel; invariant scheduling + register coloring. — decomp.me: https://decomp.me/scratch/MaiDT
10. **sub_80A6D34** — link-arena header decode; pure register coloring. — decomp.me: https://decomp.me/scratch/9myLi
11. **sub_80A6E4C** — link-arena record encode; spill / callback-veneer register choice. — decomp.me: https://decomp.me/scratch/g7FXU
12. **sub_80A6F1C** — DecodeAndVerifyArenaRecord; callback-in-high-register veneer residual. — decomp.me: https://decomp.me/scratch/h2W8F
13. **sub_80C05C8** — GmapScreen2_Loop; clean JP-vs-US coloring divergence. — decomp.me: https://decomp.me/scratch/R7AaX

**Augury / Link-Arena subsystem = sub_80A2E64 / sub_80A3300 / sub_80A3528 /
sub_80A6D34 / sub_80A6E4C / sub_80A6F1C (6 of the 13).**
