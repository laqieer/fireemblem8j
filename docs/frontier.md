...
- **SHIFTABILITY (axis #5, D296–D303): relocated 14,003 (from 7,639 at session start, +6,364)**. Cracked the FUNCTION-pointer relocation (D303): ld ORs the Thumb bit into `.4byte func` only when the addend is 0; `.4byte func + off` (off>=1) does NOT -> reproduces any function pointer exactly, no ARM/Thumb detection. Only off==0 (ptr to a Thumb fn's even start) stays raw. Levers EXHAUSTED (--fe8u-safe/--src-slices/--reprocess all 0). Remaining ~3,311 literal (auditor over/under-counts partial-de-pointer .bin + .4byte literals) = off==0 fn-starts + coincidental constants (not pointers) + fe8u-blind JP-only data (per-table RE).
