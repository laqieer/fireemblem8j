...
> [Superseded — see the latest six-axis table near the end of this doc] Historical snapshot; **two** axis lines here are stale: **(1)** MATCHING-C 99.64% / 31 still descriptive asm (live: **99.92% / 7 unmatched**), and **(2)** the SHIFTABILITY (axis #5) gate = 0 ✅ COMPLETE (D309) line just below (live: **gate = 1 (floor)** — the fe8u-confirmed coincidental constant `0x080896ED`).
...
### Six-axis scorecard refresh (axis-2 refreshed 2026-07-11; axis-3/6 refreshed 2026-07-09; verbatim source = `python3 scripts/calcprogress.py`)
| # | Axis | Value | Detail |
|---|---|---|---|
...
| 2 | Matching-C | **99.92% (8685/8692)** | 8558 `src/*.c` + 127 libc/libgcc real source; **7 still descriptive asm**. This release candidate harvests five score-0 families on top of current main's `DivinationRankSpriteUpdate`, moving the 22-function cohort to **7** (D368). |
...
> **Code-byte companion (frogress/decomp.dev `code` measure, source-form):** **99.67%** (898,452 / 901,428 bytes); **0.33% = 2,976 bytes** remains descriptive asm. Verbatim from `python3 scripts/calcprogress.py`; see D343/D368.

### Code frontier — the 7 remaining functions (axis-2 = 22 → 7; 15 matches banked/candidate)

> **2026-07-11 five-function release candidate (D368):**
...
> harvest; this branch adds the five matches above without squashing its history.
...
**Authoritative set:** `find src/nonmatching -maxdepth 1 -name '*.c'` returns **7**.
...
3. **sub_800FAD0** — GetUnitDefinitionFormEventScr; `-mjp-promote`; clean register-permutation NEAR. — decomp.me: https://decomp.me/scratch/eZzgG
4. **sub_807D3BC** — SelectSummonPos; `-mjp-promote`; extra spill-slot/frame-layout decision. — decomp.me: https://decomp.me/scratch/J1ka1
5. **sub_80A6D34** — password/header decode; pure register coloring. — decomp.me: https://decomp.me/scratch/9myLi
6. **sub_80A6F1C** — DecodeAndVerifyArenaRecord; callback-in-high-register veneer residual. — decomp.me: https://decomp.me/scratch/h2W8F
7. **sub_80C05C8** — GmapScreen2_Loop; clean JP-vs-US coloring divergence. — decomp.me: https://decomp.me/scratch/R7AaX

**Password-codec remainder = sub_80A6D34 / sub_80A6F1C (2 of the 7).**
The Play-Ranking draw/sprite pair and password encoder are now matching C.
