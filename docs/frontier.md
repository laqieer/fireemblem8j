...
> [Superseded — see the latest six-axis table near the end of this doc] Historical snapshot; **two** axis lines here are stale: **(1)** MATCHING-C 99.64% / 31 still descriptive asm (live: **99.93% / 6 unmatched**), and **(2)** the SHIFTABILITY (axis #5) gate = 0 ✅ COMPLETE (D309) line just below (live: **gate = 1 (floor)** — the fe8u-confirmed coincidental constant `0x080896ED`).
...
| 2 | Matching-C | **99.93% (8686/8692)** | 8559 `src/*.c` + 127 libc/libgcc real source; **6 still descriptive asm**. `DecodeLinkArenaRecordHeader` now matches from deterministic FE6J/FE7J cross-version reconstruction plus P14 two-address accumulator steering. |
...
> **Code-byte companion (frogress/decomp.dev `code` measure, source-form):** **99.70%** (898,732 / 901,428 bytes); **0.30% = 2,696 bytes** remains descriptive asm. Verbatim from `python3 scripts/calcprogress.py`.

### Code frontier — the 6 remaining functions (axis-2 = 22 → 6; 16 matches banked/candidate)

> **2026-07-11 deterministic cross-version match:** `DecodeLinkArenaRecordHeader`
> (`sub_80A6D34`, 280 B) is byte-exact on
> `feat/axis2-compute-80a6d34-copilot`. FE6J `func_fe6_08083180`, FE7J
> `sub_809E4D0`, and FE7U `sub_809DAB8` establish the same 13-block password
> decoder. Phase-local register roles plus P14 two-address accumulators reproduce
> the final commutative operand order, while an r1 struct alias preserves
> immediate header-field offsets. `9myLi` is SOLVED at raw score 0 and its
...
**Authoritative set:** `find src/nonmatching -maxdepth 1 -name '*.c'` returns **6**.
...
5. **sub_80A6F1C** — DecodeAndVerifyArenaRecord; callback-in-high-register veneer residual. — decomp.me: https://decomp.me/scratch/h2W8F
6. **sub_80C05C8** — GmapScreen2_Loop; clean JP-vs-US coloring divergence. — decomp.me: https://decomp.me/scratch/R7AaX

**Password-codec remainder = sub_80A6F1C (1 of the 6).**
The password header decoder and encoder are now matching C.
