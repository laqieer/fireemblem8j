...
| 4 | Named symbols | **97.13% (12,441/12,809)** | **368** placeholders; DATA-residue-split waves named catch-all `data_` blobs into fe8u-validated sub-objects (EventListScr/AnimScr/AnimSprite/REDA/UnitDefinition/EventScr), passing the earlier ~95.8% reachable-max estimate; `banim_`/`gfx_`/`snd_` remain fe8u-auto-named → structurally < 100%. **(axis-4 refreshed 2026-07-08: DATA-RESIDUE full-split wave — data_08908{228,354,790,958}, 0890{A3D4,A480,C0A4,C9BC,F2E0} + Group-3 EVENT residues → +REDA/UnitDefinition sub-objects, all `include/eventcall.h`-validated, byte-neutral `make compare` OK.)** |
