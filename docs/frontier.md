...
- **[2026-07-13, D381, issue #143 — ending-frontier six-blob wave.]** Migrated the six
...
- **`.bin` frontier: MISS=3 / FLOOR=1401 / UNCERTAIN=226** (`FE8U=../fireemblem8u python3 scripts/audit_bin_forms.py`).
  ...
  **[refreshed 2026-07-13, D376: live `docs/bin_audit.md` now reads MISS=0 / FLOOR=1415 / UNCERTAIN=30 /
  TOTAL=1445. Three content-proven chapter-map blobs (`data_0819EADC`, `data_081A00C8`, `data_081A6774`) now build
  from committed `AnotherShrineMap`, `LordsSplitMap`, and `Ch5TownMapPast` `.mar` + `.json` sources through the
  existing MARTOMAP rule; their raw and compressed bytes round-trip exactly. This closes only the three actionable
  map migrations from issue #143, not the full `.incbin` review.]**
- **Historical snapshot (superseded by D368 below): pointer-debt COMPLETION GATE = 1**
  ...
  **[CORRECTED + HARD-GATED by D378, 2026-07-13: D363's
  `sBanimEkrPopupProcNames` row was an extent-inference false positive, not a
  375 KB low-density floor. Its exact committed provider is only `0x178` bytes
  and has zero self-refs. The current **exact nonzero-extent** review set is
  **8 evidence-resolved symbols / 267 exact hit words**: four validated GBA-LZ
  streams, three DirectSound PCM payloads, and the unreferenced flat-u8
  `gUnkData_108` orphan. A path/size/hash/hit-set manifest now fails closed;
  unresolved exact-extent candidates are part of `--true-debt --gate` and
  baseline is **unresolved=0 / gate=0**. Scope is explicit: **1,074 zero-size
  opaque symbols remain unscanned (865 `AnimSprite_*` OAM/AnimScr-family
  labels)**, with no other repository-wide self-ref backstop for that
  population. The old invalid stretched-range scan found no extra candidates
  beyond these eight plus its false popup row, which is empirical reassurance,
  not an extent or completeness proof. `make shiftcheck` runs the exact-extent
  gate. This hardens the reopened checklist but does not declare issue #143
  fully closed. See decisions.md **D378**.]** **[UPDATED by D380, 2026-07-14:
  fixing `data_08B5B560`'s one false relocation makes that provider fully
  relocation-free, legitimately promoting it into this zero-size population —
  now **1,075 total / 210 non-`AnimSprite_`**, 865 `AnimSprite_*` unchanged.
  See decisions.md **D380**.]**

### Code-side raw C literal-pool lane — guarded (issue #143 lane, D377, 2026-07-13)

A repository-wide lexical sweep exposed another false-zero class: agbcc emits a
raw numeric C constant into a `.text` literal pool without `R_ARM_ABS32`, so the
existing linked relocation scanners cannot see it. Baseline inventory after
stripping comments plus string/character literals was **277 tokens / 10 files**:
**259** proven packed values (**257** `gMsgHuffmanTable`, one
`gWorldmapMapmu_1` coefficient, one OpSubtitle palette fill) and **18** live ROM
pointers across seven functions. Those 18 source occurrences collapse to
**15** unique pool slots.
...
### Remaining binary-form residuals after ending-simple assets (#143, D397-D399, 2026-07-14)

The canonical built-state audit is now **MISS=0, FLOOR=1452, UNCERTAIN=0,
TOTAL=1452**. D399 resolves the prior five-item work-list: player-rank and
the ending-CG tail no longer have raw providers; StaffReel TSA 6/7 are named
reproducible TSA sources; and the 24-byte ending-CG partial-tile fragment is
an exact size/hash-pinned evidence-backed FLOOR.

There are therefore **zero `UNCERTAIN` / actionable binary-form closure
residuals** in this concrete work-list. This does **not** mean zero opaque or
FLOOR debt: the audit still records 1,452 evidence-backed binary-form floors.
These provider integrations do not close reopened issue #143.
...
### Proven false ABS32 decodes — function interior + semantic domains fixed and gated (#143 follow-up, D380, 2026-07-13/14)

A full `fireemblem8_relocs.elf` sweep found one ROM `R_ARM_ABS32` relocation whose
final symbol type was `STT_FUNC` but whose linked value was not a function entry:
slot `0x08AC2830`, value `0x08050201`, symbol `MenuStdHelpBox`
(`S_even=0x080501CC`, interior delta `+0x35`). Consumer/type evidence resolves the
word as packed radar bytes `{1,2,5,8}`, not a callable address; the following word
at `gWorldmapRadar_2` is `{5,2,1,0}` / `0x00010205`. The source now emits the
intentional raw packed word, so a `+0x40000` relink preserves both radar words
instead of corrupting the first to `0x08090201`.

Layer 1d now reads the relocation-bearing final ELF directly. Every ROM-backed
`R_ARM_ABS32` targeting `STT_FUNC` must resolve to `S_even` or `S_even|1`; a
function interior/addend is a hard failure. Debug/non-ROM relocation sections are
excluded by target-section context, and OBJECT interiors are not findings under
this specific rule. The before/after counts are
**59,503/5,679/1 → 59,502/5,678/0**
(ROM ABS32 / FUNC targets / function-interior false relocations); the post-fix
split is **5,672 Thumb + 6 even** entries. The dedicated shifted-ROM proof checked
all **5,678/5,678** legitimate function-entry relocations: 5,656 targets moved by
`+0x40000`, while 22 targets in the deliberately fixed 0xC0-byte header remained
unchanged and were still correct. This completes only this reopened-checklist
class.

The follow-ups removed seventeen more proven semantic false relocations: eight TileAnimations3
duration/size words now emitted as `.hword 11, 2048`, plus two packed
`AREA(...,9,0,16,8)` words emitted as raw `0x08100009`, plus three
`pad_BC3A00` expressions proven to be bytes inside LZ-compressed graphics. The
documented `+0x40000` mutation changed 4 decompressed bytes in
`gLegacyUiFrameDImage` and 2,950 in the DarkBreath stream. The final four are
the `Tsa_OpAnimEphraimClose2` LZ payload word at `0x08B5B614` and three
`0x08000000` fields at `0x08FE0234`/`0x08FE14D8`/`0x08FE200C` inside a
DACS/IS-AGB debug-monitor image (not a multiboot child). The live JP
TileAnimations3 slots are `0x085C5BD8..0x085C5C10` at 8-byte stride; the
supplied `0x08A074EC..0x08A075B0` addresses are unrelated graphics in this
build, so the gate inventories the ELF rather than hardcoding them.

Layer 1d additionally fails zero-size semantic-resource ROM relocations with
addend `>=0x10000` and the proven AREA/TileAnimations3/LZ-stream packed scalars.
The AREA assertion remains active if its provider later gains an ELF size.
`pad_BC3A00` is now an exact HIGH class after consumer proof; unrelated unproven
placement anchors remain REVIEW-only. Every relocation resolving inside the
0xC0-byte cartridge header is now schema-audited: known compressed/scalar/nested
image domains fail, while only exact typed pointers or explicitly audited pinned
slots pass. The verifier's **12/12 WRONG-ID** header set is fully removed.
Final counts are **59,485 ROM ABS32, 5,678 FUNC targets, 0 function interiors,
0 header-domain relocations, 0 large-addend semantic failures,
...
| 5 | Shiftability | **gate 0; issue #166 banim residual closed; code-literal lane guarded; opaque self-ref exact-extent gate 0; proven false-ABS32 classes enforced** | A fresh `+0x40000` A/B at `2a49c9b8d` verified the four known hybrids (**172 words**) and the empirical **27-section / 85-word** rescope: **257 tagged words**, zero stale targets, **28/28** consumer checks. D377 additionally closes the raw-numeric C literal-pool blind class: **277** baseline tokens = **259** proven packed + **18** live pointers, now **15/15** relocated pool slots; the new linked-source lexical gate reports **0 unexplained**, and its `+0x40000` proof leaves all seven normalized function ranges otherwise identical. D378 hard-gates the exact-extent opaque self-ref review: **8 evidence-resolved symbols / 267 exact embedded-data hit words / 0 unresolved**; `sBanimEkrPopupProcNames` has an independently proven `0x178` provider with zero hits. The gate does **not** cover **1,075 zero-size opaque labels (865 `AnimSprite_*`, 210 other)** — updated from 1,074/209 because D380's `data_08B5B560` false-relocation fix legitimately promotes that provider into this population; no repository-wide self-ref backstop currently covers that population, and the old stretched scan finding no additional rows there is empirical history, not a completeness proof. D380 removed `gWorldmapRadar_1` plus seventeen semantic false relocations, including all **12/12** relocations resolving inside the cartridge header. Layer 1d gates function interiors, packed scalars, compressed/nested-image address domains, and unaudited header targets. Post-fix: **59,485 ROM ABS32 on this lane's own base (59,500 once stacked with D377's +15 literal-pool symbolizations), 5,678 FUNC targets, 0 interiors, 0 header-domain relocations, 0 scalar failures**; dedicated `+0x40000` A/B checked **5,678/5,678** legitimate function entries. These are scoped lane closures, not a claim that reopened issue #143 as a whole is closed. See **D368/D377/D378/D380/D383/D384/D385/D386/D387/D388/D389-D396**. **(2026-07-14, D389-D396): the menu lane (14 formerly-UNCERTAIN frontier_df4_menu files fully typed, 136-array EventScr world-map family, shop/gamerankings recut) re-verified all gates at integration with 0 weakening: D380's ptraudit ROM ABS32 total legitimately rose **59,516 -> 59,523** (+7, new EventScr/lookup/shop/gamerankings relocations) with FUNC targets (5,678), interiors (0), and header-domain (0) unchanged; D388's ProcCmd gate legitimately rose **4,701 -> 4,706** unique non-null pointer slots (+5, `gProcScr_ShopFadeIn` moves from unrecovered to `zero_size_recovered`, unrecovered count 9->8), 0 missing/malformed either side; D383's glyph gate (system 1488/1488/1488/0, talk 2085/2085/2085/0) and D378's self-ref gate (8 resolved/267 hit words/0 unresolved; 1,075 zero-size skipped, 865 `AnimSprite_*`) are both unaffected. Full `make shiftcheck` (96 graphics + 106 shiftcheck tests, each exactly once) and `make clean && make compare` both OK.)** **(2026-07-13, D383): a NEW structural gate (`scripts/shiftcheck/audit_glyph_relocs.py`, wired as `shiftcheck-glyphs`) closes a blind spot both prior gates missed — `TextGlyphs_System` carried 10 real `struct Glyph.sjisNext` pointers as unrelocated raw literals (6 `asm/data_<addr>.s` single-glyph residues too small to ever look "MIXED" to `scan_relocs.py`'s coherent-table heuristic, plus 4 `data_<addr>.c` files whose plain `u32[]` numeric initializers were invisible to `audit_pointer_classification.py`'s `.4byte`-token text scan AND never received an `R_ARM_ABS32` relocation from agbcc in the first place — a plain integer initializer never relocates, only a `&symbol` pointer expression does). Symptom: CP932 `97 CE` (緑) rendered as the fallback glyph in the shifted ROM, because the chain broke 4 hops before reaching it. Fixed all 10 (byte-identical, `make compare` OK); the new gate walks the ACTUAL 0xC0-head/`sjisNext` linked lists (schema-known fields only, never a generic "every interior word is a pointer" rule) and requires a real relocation at each non-null word: fixed-tree baseline is now **system 1488/1488/1488/0, talk 2085/2085/2085/0** (unique/slots/relocated/missing). A shifted-ROM A/B extension proves every reachable glyph's links track `+shift` and its 0x44 payload bytes stay identical (0 mismatches on both tables), directly confirming 緑 is reachable at both the baseline and shifted addresses. Gate count stays 0 (no axis-5 number change) — this is a blind-spot closure, the same shape as D368's banim closure, not a new residual.)** **(2026-07-13, D384): a SECOND structural gate (`scripts/shiftcheck/audit_procscr_relocs.py`, wired as `shiftcheck-procscr`) closes the identical blind-spot CLASS in a second consumer — `struct ProcCmd` battle-anim scripts. Three `PROC_NAME` slots in `frontier_df4_banim_a.c` (`ProcScr_efxThunderBG`/`BGCOL`/`efxFireOBJ`) held raw `(const void*)0x08..` literals instead of the file's own established `Sym+off` idiom. Fixed all 3 (byte-identical); the new gate audits every GLOBAL source-declared `struct ProcCmd NAME[]` definition (511 found, 506 resolve in the ELF) against the pointer-bearing opcode set from `include/proc.h`: fixed-tree baseline is **506 arrays / 4,798 records / 2,903 pointer slots, 2,903 relocated, 0 missing, 0 malformed**. Shifted A/B proves all 2,903 pointer fields track `+shift` (0 mismatches). Fixing the literals also surfaced a genuine false-positive in the pre-existing `scan_offsets.py` Layer-1b gate (two of the three newly-relocated targets happened to land exactly on a non-nameable compiler-local `static` symbol's start in an unrelated file) — refined that scanner to a 3-bucket HIGH/REVIEW/LOCAL-TARGET classification so the real "wrong-base" bug class it targets stays gated while this unavoidable coincidence does not. Gate count stays 0.)** **(2026-07-13, D385): D384's gate was INCOMPLETE — its source-declaration enumeration missed 3 MORE disguised `struct ProcCmd` scripts declared as plain `u32[]` in the same file (`data_08601748`/`data_086019E0`/`data_086019F8`, each cast `(const struct ProcCmd *)` and `Proc_Start()`-ed from a different TU), holding the identical raw-literal `PROC_NAME` bug (6 total missing, not 3). Fixed all 3 (carved in-place to typed `struct ProcCmd[]`, byte-neutral); broadened `audit_procscr_relocs.py` to strict-decode a ProcCmd prefix from EVERY ROM object symbol's raw bytes (dedup by address, not name — this also retires the earlier GLOBAL-only restriction, since address-keying has no `static`-name-collision ambiguity to route around) instead of trusting source-declared C types. Fixed-tree baseline: **635 script prefixes, 6,161 records, 3,719 pointer slots, 3,719 relocated, 0 missing, 0 malformed**; shifted A/B 0 mismatches. Gate count stays 0 — a third recurrence of the identical "declared C type doesn't match actual byte schema" trap (D383 glyph `u32[]`, D384 mistyped extern, D385 mistyped definition), now closed at the byte-decode level for both consumers.)** **(2026-07-14, D386): D385's byte-decode gate had a real object-extent defect — it never bounded a candidate's decode to its OWN `nm` size, so scripts legitimately ending in a terminal control-transfer opcode with no trailing `PROC_END` (demonstrated: `ProcScr_MapAnimPoisonDmg` + 8 `PROC_JUMP`-ending siblings) were walked straight through into the NEXT unrelated object (68 overlaps / 22 distinct nested starts ROM-wide). Fixed: every decode is now bounded to `[addr, addr+size)`; the terminal-opcode set was corrected from a 1-opcode guess to 4 opcodes verified via source/disassembly (`PROC_JUMP`/`PROC_GOTO`/`PROC_BLOCK`/`PROC_REPEAT`), applied only when truly the object's last record (not mid-object); NOLOAD "phantom placement" candidates are trusted when they decode cleanly and only excluded on failure (an earlier draft's blanket NOLOAD exclusion regressed and silently hid `ProcScr_efxThunderBG`'s already-fixed finding); and genuine same-section split continuations (`gProcScr_GameControl`, `gProcScr_CharacterEndings`) are now empirically verified, not guessed. Re-run against a scratch pre-fix rebuild reproduces exactly the same 6 missing slots and 0 elsewhere; fixed tree: **631 script prefixes, 5,458 records, 3,330 pointer slots, 3,330 relocated, 0 missing, 0 malformed, 2 verified split-continuations** (closer to the externally-reported 615 than D385's 635). Shifted A/B re-verified, 0 mismatches. See **D386**; integrator must renumber D383-D386 against `main`'s actual sequence.)** **(2026-07-14, D387): D386's `nm -S` symbol loader silently dropped every 3-field zero-size line (`ADDR TYPE NAME`, no size column at all) behind a `len(parts) < 4` guard -- 262,885 of 285,980 total lines, so `zero-size (skipped): 0` was a false report hiding 11,473 real never-considered ROM addresses. Fixed: both the 4-field sized and 3-field zero-size `nm -S` line shapes are now parsed explicitly (factored into a pure, directly-unit-tested `parse_nm_s_output`). Added a deterministic zero-size backstop (the SAME `extended_bound` mechanism already used for split-continuations, generalized to size=0): a zero-size candidate is promoted only if extending its decode up to the next known candidate address(es) reaches a full valid termination. Recovers **364** previously-invisible ProcCmd scripts (confirmed to include `data_085C66D8`), all fully relocation-audited with **0 missing, 0 malformed** among them. Fixed-tree result: **34,214 candidate addresses; 11,109 zero-size (honest remaining blind spot, pinned by name -- only 8 Proc-suggestively-named, none of them genuine ProcCmd data on inspection, e.g. `sProcessCmdTable` is the opcode dispatch table, not script records); 364 zero-size recovered; 995 script prefixes, 7,788 records, 4,730 pointer slots, 4,730 relocated, 0 missing, 0 malformed, 2 verified split-continuations** (unchanged from D386). Also corrects D386's 635-vs-622 count typo (above) and confirms the NOLOAD-exclude-on-failed-decode branch has 0 real-ROM occurrences in the current tree (`NOLOAD (skipped): 0`) -- retained as tested, synthetic-only defensive behavior, not a demonstrated live case; `gProcScr_GameControl` continues to be resolved via verified split-continuation, not NOLOAD exclusion. Gate count stays 0. See **D387**; integrator must renumber D383-D387 against `main`'s actual sequence.)** **(2026-07-14, D388): D387's zero-size backstop reintroduced the exact D386 cross-object annexation bug through a different path -- reusing the multi-hop, full-neighbor-size `extended_bound` for ZERO-size candidates (which own no bytes of their own, unlike a genuinely truncated nonzero-size candidate) let it read straight through an entire independently-sized neighbor whenever that neighbor's OWN bytes also happened to decode validly. Demonstrated real regressions: `gProcScr_PhaseIntroUnk` (truly 16B/2 records) swallowed the entirety of the following `gProcScr_PhaseIntroText` (72B) into one bogus 88B/11-record blob; `gProcScr_ShopFadeIn` walked through an entire sibling object, annexing 56B that was never its own. Fixed: the zero-size backstop's ceiling is now HARD-bounded at the next nonzero-size candidate's START (never any part of its span, via new `first_nonzero_neighbor`/`zero_size_boundary` helpers); split-continuation keeps reading into its ONE verified neighbor but never a second, and a NEW general `crossed_nonzero_objects` audit rejects (into a new `boundary_rejected` bucket) ANY promotion whose consumed range crosses more than one independently-sized object, for both mechanisms. Also made the relocation-completeness and shifted-A/B denominators physical-address-truthful: a new `unique_pointer_slots` deduplicates pointer-bearing slot events by ROM source address across overlapping/nested prefixes (fixing the previously-unexplained 4,743-vs-4,730 discrepancy at its root) and separates NULL pointer-bearing fields into their own count. Fixed-tree result: **994 prefixes, 7,772 records (non-unique), unique_non_null_pointer_slots=4,701, unique_relocated=4,701, unique_missing=0, null_pointer_bearing_fields=13, 0 malformed, 2 verified split-continuations, 0 boundary-rejected**; `ShopFadeIn` moves from recovered back to the honest zero-size-unrecovered inventory (now 9 names, was 8). Pre-fix re-verification still reproduces exactly the same 6 unique missing slots; fixed tree 0. 71 focused unit tests (6 new: 3 boundary regressions, 1 crossing-audit, 2 dedup), up from 65. Shifted A/B re-verified: glyph 0 mismatches (緑 confirmed reachable), ProcCmd 4,701 unique non-null slots + 13 null fields checked, 0 mismatches. Gate count stays 0. See **D388**; integrator must renumber D383-D388 against `main`'s actual sequence.)** |
...
## issue143 TSA-outlier / multi-resource migration (D379, 2026-07-14)
...
`scripts/audit_bin_forms.py` regenerated: net **+3 FLOOR / +3 TOTAL** (-1 old
monolith, +4 new raw-TSA/tilemap FLOOR sources), `MISS` stays **0**, `UNCERTAIN`
unchanged. Integrated onto `main` (after the D376 map and D378 self-ref lanes):
**MISS=0 / FLOOR 1417→1420 / UNCERTAIN 28 (unchanged) / TOTAL 1445→1448**.
Issue #143 stays open pending the wider `.incbin` review. See decisions.md
**D379**.
