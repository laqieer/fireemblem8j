...
| 2 | Matching-C | **99.93% (8686/8692)** | 8559 `src/*.c` + 127 libc/libgcc real source; **6 still descriptive asm**. The `sub_800FAD0` P14 candidate moves the 22-function cohort to **6**. |
...
> **Code-byte companion (frogress/decomp.dev `code` measure, source-form):** **99.72%** (898,916 / 901,428 bytes); **0.28% = 2,512 bytes** remains descriptive asm. Verbatim from `python3 scripts/calcprogress.py`.

### Code frontier — the 6 remaining functions (axis-2 = 22 → 6; 16 matches banked/candidate)

> **2026-07-11 `sub_800FAD0` score-0 candidate:** `GetUnitDefinitionFormEventScr`
> now compiles byte-exact under its existing per-TU `-mjp-promote` requirement.
> The decisive P14 shape widens the stacked fifth argument to ABI `int`, delays
> its `(s8)` re-narrow until after `arg3` receives `[sp,#0x40]`, keeps the RNG
> selection count at `[sp,#0x44]`, and reads the final build flag through r5.
> Linked range compare, normal and cold `make compare`, shiftcheck, CBMC
> (0/374), and 200-trial differential testing pass. Owned family `eZzgG`
> exposes effective score 0 through `match_override`.
...
4. **sub_80A6D34** — password/header decode; pure register coloring. — decomp.me: https://decomp.me/scratch/9myLi
