...
- **SHIFTABILITY (axis #5, D296/D297): 80.44% literal / 82.03% REAL** (11,670 relocatable; **2,838 hardcoded, of which only 2,556 are REAL pointers** — the other 282 are coincidental constants in graphics/sound blobs, not pointers, never relocatable). Driven from ~38% this session via the **fe8u reloc oracle** (`scripts/fe8u_ptr_offsets.py` + `repoint_table.py --fe8u-safe`): converts a table ONLY at fe8u-confirmed pointer byte-offsets (+ EXACT words), with stride extrapolation for JP-divergent records and self-validating offset alignment (gClassData: 847 real pointers convert, 15 coincidental stay raw). Handles both `_ref` .c and asm-incbin wirings. Metric-accuracy fix: orphaned `.bin` of already-relocated structs (gClassData via data_classes.c) no longer overcounted. **Remaining ~2,556 real debt = mostly un-named generic `data_*` blobs** (un-mapped → fe8u can't gate by name) + JP-divergent event scripts. **Next lever:** map `data_<JPaddr>` → US addr (region shift from us_jp_funcmap anchors) → fe8u symbol → offsets, gated by JP/fe8u offset-alignment self-validation (safe: misalignment = skip). Then the JP-divergent EventScr bytecode (parse event-command format for pointer args). Per-table, multi-session. Raw `.bin`/`​.byte` data dumps freeze pointers as constants → on any rebuild that shifts a section they dangle and the game crashes; a real decomp must relocate every pointer. **The whole program is MECHANICAL: 0 dangling** (every pointer target already has an ELF symbol), split 3,167 EXACT (`(u32)&Sym`) + 9,081 INTERIOR (`(u32)&Sym + off`). Tool: `scripts/repoint_table.py <name>` rewrites a residual `_ref` .c from `INCBIN_U8` → `const u32 NAME[] = { (u32)&Sym, … }`; agbcc lowers it to a `.word` relocation so the ROM stays byte-identical (`make compare` is the byte-exact oracle) yet shiftable. Function pointers resolve correctly (FE8 stores them odd; ld sets the thumb bit). Audit/track: `scripts/audit_pointers.py [--metrics]`. Proven on gWMPathData (53 ptrs). **Plan:** Track A = pure pointer / fn-ptr LUTs auto-repointed + make-compare-gated + orphaned `.bin` pruned; Track B = struct/event/proc tables ported from fe8u typed defs (also retires axis #6 + improves naming). Risk to guard: a non-pointer field coincidentally in ROM range — auto-repoint only pure tables; cross-ref fe8u for struct field offsets. See D296.
