...
- **SHIFTABILITY (axis #5, D296–D302): relocated 13,686 (from 7,639 at session start, +6,047)**. NOTE the literal `audit_pointers.py` count (~3,809) UNDERCOUNTS: it scans INCBIN `.bin` only, missing raw `.4byte 0xNN` literals stuck in already-de-pointered `__asm__` blocks. `repoint_table.py --reprocess` re-processes those (label NAME -> word i at NAME+i*4 -> JP addr; convert EXACT or fe8u-confirmed; skip genuine functions = ld Thumb-bit-on-ABS32 is inconsistent). Levers EXHAUSTED: --fe8u-safe, --src-slices (incl SECTION form), --reprocess all return 0. TRUE-real-pointer shiftability ~94%+ (`audit_pointers.py --true-debt`). The literal/non-graphics figures over-count because the byte auditor cannot tell a pointer from a coincidental `0x08xxxxxx` data value; fe8u can. Of the 4,619 non-graphics hardcoded words, **only ~1,631 are TRUE real-pointer debt** (115 fe8u-confirmed-convertible + 1,516 fe8u-blind-unknown / JP-divergent); the other ~2,988 are fe8u-confirmed coincidental constants (not pointers, never relocatable). So real-pointer shiftability is ~88% (12,253 relocated / ~13,884 real pointers). The remaining genuine work is the **~1,516 fe8u-blind-unknown** (JP-only data or regions with no funcmap anchor) — needs per-table JP structure analysis / better US-JP data mapping. **D299:** earlier the fe8u-oracle-reachable de-pointering — `_ref` tables + sliced `src/data/<x>/<x>.c` sub-symbols via name + region-shift address map — is now EXHAUSTED. The remaining 4,843 is fe8u-UNCONFIRMABLE: a mix of coincidental constants in logic-named tables (NOT real pointers, so the true real debt is lower) + JP-divergent real pointers (different event/struct content than US) that need per-table JP structure analysis — e.g. EventScr bytecode parsing — to confirm which words are pointers. Earlier figures up to "82%" were inflated by ineffective asm-placeholder edits (D299, corrected). — the other 285 are coincidental constants in graphics/sound blobs, not pointers, never relocatable). Driven from ~44% this session via the **fe8u reloc oracle** (`scripts/fe8u_ptr_offsets.py` + `repoint_table.py --fe8u-safe`): converts a table ONLY at fe8u-confirmed pointer byte-offsets (+ EXACT words), stride extrapolation for JP-divergent records, self-validating offset alignment (gClassData: 847 real pointers convert, 15 coincidental stay raw), and data_<JPaddr> address-mapping. **D299 INTEGRITY NOTE:** an earlier claim of "82%" was inflated — the "asm-incbin" batches edited EXCLUDED `asm/dat_*.s` placeholders (dead; residual symbols link from `src/data/<name>/<name>.c`), and the auditor's asm-incbin liveness was fooled. Reverted; liveness now gates on the LINKED src/data source (matches the ld object list exactly). **Remaining ~5,238 real debt = un-named `data_*` blobs (need US-JP address mapping) + sliced `src/data/<name>/<name>.c` tables + JP-divergent event scripts.** **Next mechanism:** rewrite the LINKED `src/data/<name>/<name>.c` sliced `INCBIN_U8(bin,off,len)` sub-symbols (each gated by its own fe8u offsets) — NOT the asm placeholder (D299). Verify effectiveness against the linker object list, never against a file merely existing. Per-table, multi-session. Raw `.bin`/`​.byte` data dumps freeze pointers as constants → on any rebuild that shifts a section they dangle and the game crashes; a real decomp must relocate every pointer. **The whole program is MECHANICAL: 0 dangling** (every pointer target already has an ELF symbol), split 3,167 EXACT (`(u32)&Sym`) + 9,081 INTERIOR (`(u32)&Sym + off`). Tool: `scripts/repoint_table.py <name>` rewrites a residual `_ref` .c from `INCBIN_U8` → `const u32 NAME[] = { (u32)&Sym, … }`; agbcc lowers it to a `.word` relocation so the ROM stays byte-identical (`make compare` is the byte-exact oracle) yet shiftable. Function pointers resolve correctly (FE8 stores them odd; ld sets the thumb bit). Audit/track: `scripts/audit_pointers.py [--metrics]`. Proven on gWMPathData (53 ptrs). **Plan:** Track A = pure pointer / fn-ptr LUTs auto-repointed + make-compare-gated + orphaned `.bin` pruned; Track B = struct/event/proc tables ported from fe8u typed defs (also retires axis #6 + improves naming). Risk to guard: a non-pointer field coincidentally in ROM range — auto-repoint only pure tables; cross-ref fe8u for struct field offsets. See D296.
- **ASSET EDITABILITY (axis #6, D296):** 1.06 MB opaque raw-incbin data; **759 KB is structured/logic data that should be typed C** (event/proc scripts, struct tables, LUTs), 303 KB is legit graphics binary (exempt, like fe8u `.4bpp`/`.bin`). Retired in lockstep with Track B above.
