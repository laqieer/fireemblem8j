...
| 2 | Matching-C | **99.92% (8685/8692)** | 8558 `src/*.c` + 127 libc/libgcc real source; **7 still descriptive asm**. This release candidate harvests five score-0 families on top of current main's `DivinationRankSpriteUpdate`, moving the 22-function cohort to **7** (D369). |
...
> **Code-byte companion (frogress/decomp.dev `code` measure, source-form):** **99.67%** (898,452 / 901,428 bytes); **0.33% = 2,976 bytes** remains descriptive asm. Verbatim from `python3 scripts/calcprogress.py`; see D343/D369.
