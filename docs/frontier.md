...
| 2 | Matching-C | **99.94% (8687/8692)** | 8560 `src/*.c` + 127 libc/libgcc real source; **5 still descriptive asm**. The five-function D369 harvest, D370 `DecodeAndVerifyArenaRecord`, and the serialized `DecodeLinkArenaRecordHeader` P14 promotion move the 22-function cohort to **5**. |
...
> **Code-byte companion (frogress/decomp.dev `code` measure, source-form):** **99.72%** (898,944 / 901,428 bytes); **0.28% = 2,484 bytes** remains descriptive asm. Verbatim from `python3 scripts/calcprogress.py`; see D343/D369/D370.

### Code frontier — the 5 remaining functions (axis-2 = 22 → 5; 17 matches banked)
...
**Authoritative set:** `find src/nonmatching -maxdepth 1 -name '*.c'` returns **5**.
...
5. **sub_80C05C8** — GmapScreen2_Loop; clean JP-vs-US coloring divergence. — decomp.me: https://decomp.me/scratch/R7AaX
