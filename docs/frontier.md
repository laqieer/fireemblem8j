...
### False function-relocation decode — one proven instance fixed; class gate added (#143 follow-up, D380, 2026-07-13)
...
class; it does **not** claim issue #143 is fully closed.
...
| 5 | Shiftability | **gate 0; issue #166 banim residual closed; code-literal lane guarded; opaque self-ref exact-extent gate 0; function-interior ABS32 class enforced** | A fresh `+0x40000` A/B at `2a49c9b8d` verified the four known hybrids (**172 words**) and the empirical **27-section / 85-word** rescope: **257 tagged words**, zero stale targets, **28/28** consumer checks. D377 additionally closes the raw-numeric C literal-pool blind class: **277** baseline tokens = **259** proven packed + **18** live pointers, now **15/15** relocated pool slots; the new linked-source lexical gate reports **0 unexplained**, and its `+0x40000` proof leaves all seven normalized function ranges otherwise identical. D378 hard-gates the exact-extent opaque self-ref review: **8 evidence-resolved symbols / 267 exact embedded-data hit words / 0 unresolved**; `sBanimEkrPopupProcNames` has an independently proven `0x178` provider with zero hits. The gate does **not** cover **1,074 zero-size opaque labels (865 `AnimSprite_*`, 209 other)**; no repository-wide self-ref backstop currently covers that population, and the old stretched scan finding no additional rows there is empirical history, not a completeness proof. D380 removed the one proven false function relocation (`gWorldmapRadar_1`) and made Layer 1d reject every ROM `R_ARM_ABS32`→`STT_FUNC` value except exact even/Thumb entry. Post-fix: **59,502 ROM ABS32, 5,678 FUNC targets, 0 interiors**; dedicated `+0x40000` A/B checked **5,678/5,678** legitimate entries. These are scoped lane closures, not a claim that reopened issue #143 as a whole is closed. See **D368/D377/D378/D380**. |
