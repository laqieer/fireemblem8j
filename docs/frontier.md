...
- **Historical snapshot (superseded by D368 below): pointer-debt COMPLETION GATE = 1**
  ...
  **[CORRECTED + HARD-GATED by D378, 2026-07-13: D363's
  ...
  and has zero self-refs. The current positive-size review set is **8
  evidence-resolved symbols / 267 exact hit words**: four validated GBA-LZ
  ...
  unresolved self-ref candidates are part of `--true-debt --gate` and baseline
  is **unresolved=0 / gate=0**. `make shiftcheck` runs this gate. This hardens
  the reopened checklist but does not declare issue #143 fully closed. See
  decisions.md **D378**.]**
...
| 5 | Shiftability | **gate 0; issue #166 banim residual closed; code-literal lane guarded; opaque self-ref unresolved=0** | A fresh `+0x40000` A/B at `2a49c9b8d` verified the four known hybrids (**172 words**) and the empirical **27-section / 85-word** rescope: **257 tagged words**, zero stale targets, **28/28** consumer checks. D377 additionally closes the raw-numeric C literal-pool blind class: **277** baseline tokens = **259** proven packed + **18** live pointers, now **15/15** relocated pool slots; the new linked-source lexical gate reports **0 unexplained**, and its `+0x40000` proof leaves all seven normalized function ranges otherwise identical. D378 now hard-gates the separate opaque self-ref review: **8 evidence-resolved symbols / 267 exact embedded-data hit words / 0 unresolved**; `sBanimEkrPopupProcNames` is absent because its exact `0x178` provider has zero hits. Any new candidate or evidence drift fails `make shiftcheck`. These are scoped lane closures, not a claim that reopened issue #143 as a whole is closed. See **D368/D377/D378**. |
