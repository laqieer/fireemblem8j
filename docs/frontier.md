...
### Six-axis scorecard refresh (2026-07-03, verbatim from `python3 scripts/calcprogress.py`; supersedes every older "Current state" axis line above; **axis-3 & axis-6 refreshed 2026-07-09 — D358 data-extraction wave**)
| # | Axis | Value | Detail |
|---|---|---|---|
...
1. **sub_8001570** — AddAttr2dBitMap (mapanim_eventcall); FLAGS `-mjp-promote`; reg-allocator-internal save-order tie-break (levers §7 / D284); a 67k-iter permuter reported "score 0" but that scorer is register-blind (false-green). — decomp.me: https://decomp.me/scratch/ABitG (start here to help match; WIP scratch, not a solved match).
2. **sub_800A34C** — worldmap coeff/segment-search; agbcc whole-function reg-coloring wall; `-fno-gcse` gives EXACT size 0x248 but **536/584 bytes differ** (pts<->coeffA coloring swap) — the unreliable-"EXACT"-annotation example. — decomp.me: https://decomp.me/scratch/ABtKz (start here to help match; WIP scratch, not a solved match).
3. **sub_800A594** — sibling of sub_800A34C; spill-decision / reg-coloring wall (JP spills `count`, keeps `pts` in r7 — whole-fn renumber); permuter base 7215 → best 5795 (never near 0). — decomp.me: https://decomp.me/scratch/Sp10a (start here to help match; WIP scratch, not a solved match).
4. **sub_800E1FC** — Event18_ColorFade; FLAGS `-mjp-promote`; 204/204 mnemonic-identical, ~95B pure spill/reg-alloc residual (extra spill: red→[sp,#8] vs JP keeps it in sl) — the flagship spill-NEAR (§7). — decomp.me: https://decomp.me/scratch/nlJVc (start here to help match; WIP scratch, not a solved match).
5. **sub_800FAD0** — GetUnitDefinitionFormEventScr (fe8u eventscr.c:2376); FLAGS `-mjp-promote`; clean register-permutation NEAR (colors `i` in callee-saved r7 vs JP arg2 in caller-saved). — decomp.me: https://decomp.me/scratch/eZzgG (start here to help match; WIP scratch, not a solved match).
6. **sub_8057F80** — PrepareBattleGraphicsMaybe; ✅ **MATCHED 2026-07-07** (community decomp.me fork https://decomp.me/scratch/rtMN6, agbcc -O2 + register pins; graduated to `src/PrepareBattleGraphicsMaybe.c`). The prior "genuinely region-different, byte match out of scope" verdict was **half-wrong**: the size delta is the US-only Manakete/Demon-King tail, but the `[R1]/[R2]` "JP uses GetSelectTargetCount" claim was an ambiguous-funcmap misID — JP uses `GetBanimLinkArenaFlag` (same as US). — decomp.me base scratch (superseded): https://decomp.me/scratch/WHtsM.
7. **sub_807C8DC** — AdjustNewUnitPosition; FLAGS `-mjp-promote` (s8 ASR on ix/iy); if-branch register coloring + unit param spilled to r8 (hi reg) → cascading diffs; 3-reg loop-counter rotation. — decomp.me: https://decomp.me/scratch/taZrH (start here to help match; WIP scratch, not a solved match).
8. **sub_807D3BC** — FLAGS `-mjp-promote` (s8); JP takes an extra 4-byte spill slot [sp,#0x84] (spills the outer-loop var) → frame 140 vs 144; permuter base 1695, plateau ~875. — decomp.me: https://decomp.me/scratch/J1ka1 (start here to help match; WIP scratch, not a solved match).
9. **sub_80A2E64** — Augury/Link-Arena; plain `-O2` (the header's `-fno-gcse` note is WRONG); reg-alloc / IV-scheduling tie-break (spill proc→[sp,#4] or permute r8<->r9 / sl<->r9 / r4<->r5<->r6). — decomp.me: https://decomp.me/scratch/qksQG (start here to help match; WIP scratch, not a solved match).
10. **sub_80A3300** — Augury; sibling of sub_80A2E64; reg-coloring tie-break; canon-diff 27/106 insns, permuter 285 from base 1850 (spill i+1: `sub sp,#12` vs `#8`). — decomp.me: https://decomp.me/scratch/ENay1 (start here to help match; WIP scratch, not a solved match).
11. **sub_80A3528** — Augury; FLAGS `-Os` (plain `-O2` byte-identical too); invariant-scheduling + reg-coloring (won't pre-schedule the >0x1f field-offset address adds); a 9797-iter permuter pass did not converge. — decomp.me: https://decomp.me/scratch/MaiDT (start here to help match; WIP scratch, not a solved match).
12. **sub_80A390C** — Augury; zero-fills a 0x18-byte `struct GameRankSaveData` scratch (sub_80D6370 = CpuSet); reg-coloring + field-pointer materialization (reg-pressure); <10-byte tail, outside the single-pass permuter budget. — decomp.me: https://decomp.me/scratch/xYHce (start here to help match; WIP scratch, not a solved match).
13. **sub_80A6D34** — Augury/Link-Arena; FLAGS `-O2` (NOT `-mjp-promote`); PURE reg-coloring — the instruction SEQUENCE is identical; decomp-permuter territory. — decomp.me: https://decomp.me/scratch/9myLi (start here to help match; WIP scratch, not a solved match).
14. **sub_80A6E4C** — Augury/Link-Arena; spill-decision / reg-coloring NEAR (§7 / D284); `-O2`: 212B vs 208B JP (4B larger literal pool from extra reg-reloads), 152 differing bytes. — decomp.me: https://decomp.me/scratch/g7FXU (start here to help match; WIP scratch, not a solved match).
15. **sub_80A6F1C** — Augury/Link-Arena; FLAGS `-O2`; blocking class = callback-in-a-high-reg via a `_call_viaN` veneer (coloring-only residual); beyond the ~10-byte cheap-permuter window. — decomp.me: https://decomp.me/scratch/h2W8F (start here to help match; WIP scratch, not a solved match).
16. **sub_80C05C8** — GmapScreen2_Loop; CLEAN reg-coloring NEAR (structure byte-exact); the SAME C compiles to a MATCHING coloring on the US axis (fe8u's checked-in worldmap_screen2.s + the US ROM MATCH) but a different JP coloring — a genuine agbcc JP-vs-US divergence. — decomp.me: https://decomp.me/scratch/R7AaX (start here to help match; WIP scratch, not a solved match).
