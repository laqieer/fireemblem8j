...
- **ASSET EDITABILITY (axis #6) — REALIGNED to fe8u SOURCE forms (D310/D311, user-directed).** Editability is judged by the committed SOURCE FORM, not the raw-incbin byte count (a .bin moved into graphics/ passes make compare but isn't editable). DONE in editable fe8u form: 166/166 EventScr -> EVENT_* macros, all ProcScr -> PROC_*, 155 UnitDef -> typed struct, 21 btl_bg -> .png (FETSATOOL), 122 palettes -> .pal/.agbpal (109 bit-15->.agbpal), 74 chapter maps -> .mar/.json/.S, 439 PCM samples -> .aif, pixel gfx -> .png. Standard = docs/asset_forms.md. MUSIC DONE (D311): 72/72 songs -> editable .mid (self-contained, mid2agb from fe8u .mid + remnant re-carve) + 86 voicegroups -> editable .s (only 2 region-diff: vg036/076). Remaining music CEILINGS: gSongTable (needs ~520 SFX/voice songs decompiled), 11 unnamed voicegroup gap blobs (need RE), 2 region-diff voicegroups -- all self-contained .bin. fe8u-parity binary floor: TSA/.map.bin tilemaps, compressed region-diff gfx.
