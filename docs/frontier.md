...
- **MATCHING-C: 99.38%** (8626/8680 JP funcs; **54 still descriptive asm**). The -fno-gcse-enhanced all-levers re-examination keeps cracking "ceilings" (UnitList_PutRow/HandleTurnRecordText/806A41C +3, both prior-NEAR verdicts WRONG: msgid shifts + JP text layout, not reg-coloring). Permuter fleet (8 hardest) grinding in parallel + decomp.me scratches posted. New levers documented D284-286.. Final all-levers campaign +5 (incl LAPhaseIntro_Init 804E038 — overcame the rodata-in-shared-gap BLOCKED verdict via a gap split; 80A3E24, EventA8_WmUnitMoveFree, fflush). The carveable veins are now EXHAUSTED across 4 all-levers passes + 5 permuter + regpin campaigns. **The genuine remaining floor = agbcc SPILL-DECISION/codegen-version residual** (the JP ROM original agbcc spills to different stack slots than the -mjp-promote agbcc; documented byte-CLOSE near-matches that no source permutation/register-pin/flag forces, permuter plateaus 4155/810 etc. never 0 — see AdjustNewUnitPosition 807C8DC, Event18_ColorFade 800E1FC, 80CAEF4, 806A41C). ~42 Thumb spill-ceilings + ~9 region-diff newlib (malloc/dtoa/locale custom JP build) + ~5 fe8u-own/newlib-version non-matching. Path to 100% = match the original JP agbcc reg-allocator (toolchain work) or lucky permuter seeds, NOT more carving. +9 newlib/libgcc lib-links (_read_r/_write_r/_close_r/_lseek_r/_fstat_r/_sbrk_r via an errno@0x03006798 bind + _mbtowc_r + __lshrdi3/__negdi2); the malloc/dtoa/locale cluster is region-different JP-custom (not lib-linkable). +4 all-levers. +51 real-source ports: m4a_1.s (m4a sound engine, fe8u's real `src/m4a_1.s`), arm.s + arm_call.s (13 ARM + 5 thumb, fe8u real source) — the "ARM/m4a ceiling" was just descriptive-incbin TUs that fe8u ships as real `src/*.s`. Badges (decomp.dev + FE Portal) verified live-honest at 99.17% functions / 94.91% code-bytes (Code badge excludes lib like fe8u; ~96.7% incl lib). ⚠️ **METRIC FIXED (D283):** the matching-C axis previously divided by the US function count (8528), reading an inflated 99.99% while 125 JP functions were still asm. Now it divides by the **true JP function total** = decompiled + still-as-asm (counted by exported uppercase-T entries; gbadisasm `_<hex>` internal labels filtered — that also removed 23 internal labels wrongly inflating the src numerator). 100% now means ZERO functions in asm. By CODE BYTES: **95.93% in src (4.07% = 36,728 bytes still descriptive asm)**. The ALL-LEVERS re-examination campaign hit **8/14 (57%)** — definitively proving the "reg-coloring ceiling" verdicts were mass-wrong: PutUnitSpriteIconsOam/Tactician_InitScreen/ClassIntro_Init/OpAnimFaceMontageBegin + 4 more, all via -mjp-promote + identity-rederive + named-bitfields + register-pins. Batch 2 (14 largest, incl sub_8057F80 2936B) running. Earlier this session:
