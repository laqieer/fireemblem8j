...
### frontier_df4_menu 14-file UNCERTAIN class — full coverage, MISS=0/UNCERTAIN=0 for the menu lane (issue #143 menu lane, D378-D384, 2026-07-14)
...
Final source-coverage pass (D382/D383/D384): all four remaining blobs with a
full original opaque `.bin` (001/021/027/037) had their duplicated bytes
removed. This took two attempts — the first (D382) correctly eliminated
...
- **001** (`frontier_df4_menu_001_A588C0.bin`, originally 6812 B): D382 first
  trimmed this to a 4632 B "floor", concluding no gbagfx compressor preset
  reproduced its LZ77 stream. **This was wrong (D384's correction).** The
...
See decisions.md **D378** (Phase A/B + shop/arena/rankings), **D379** (full
...
**D380** (136-array boundary correction), **D381** (lookup type-safety
follow-up/dead-extern cleanup), **D382** (source-coverage pass: 021/027/037
...
**D383** (blob017 `.tsa.bin` -> `*_map.bin` rename to satisfy the
structurally-discovered headerless-map convention), and **D384** (blob001's
...
sequence; the exact integration mapping onto `origin/main`'s D388 tip is
**D378->D389, D379->D390, D380->D391, D381->D392, D382->D393, D383->D394,
D384->D395** (the integrator should apply this renumbering, not this lane).
...
| 5 | Shiftability | **gate 0; issue #166 banim residual closed; code-literal lane guarded** | A fresh `+0x40000` A/B at `2a49c9b8d` verified the four known hybrids (**172 words**) and the empirical **27-section / 85-word** rescope: **257 tagged words**, zero stale targets, **28/28** consumer checks. D377 additionally closes the raw-numeric C literal-pool blind class: **277** baseline tokens = **259** proven packed + **18** live pointers, now **15/15** relocated pool slots; the new linked-source lexical gate reports **0 unexplained**, and its `+0x40000` proof leaves all seven normalized function ranges otherwise identical. These are scoped lane closures, not a claim that reopened issue #143 as a whole is closed. See **D368/D377**. |
