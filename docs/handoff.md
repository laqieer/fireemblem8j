...
**+124 now (7891, 92.53%).** D253 GetBanimTerrainGround (named-asm, inline `_GetBanimTerrainGround` tileset-switch
helper). D252 SioMenu_RestartGraphicsMaybe (coddog region-same NEAR: StartFace FID_ANNA→+1, D203). The
BATCH/PORTABLE VEINS ARE NOW EXHAUSTED (verified this session): stranded-section (0 region-same still-asm),
coddog region_same_candidates (125/144 carved; the 19 leftovers = 11 libc + 2 codegen-dead-end
[ply_memacc switch diff89, PidStatsAddSquaresMoved diff59] + 5 event-script-asm-in-US + 1 carved), named-asm
...
cast-hoist sign-ext. SKIP lsr↔asr / reg-alloc / switch-lowering (dead-ends). Against honest 8209 ceiling,
matching-C is ~96.1%. The 5 event-script fns (HandleCh5xUnits/RouteSplit/SetupAutoGeneratedUnitdefs) need
full JP hand-decomp (asm even in fe8u).
