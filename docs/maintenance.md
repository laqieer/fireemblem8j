...
(its addresses and membership are not pruned as work lands). It is a diagnostic
artifact, **not** the work list. Reconcile any scope discrepancy against
`scripts/calcprogress.py` and the live list in `docs/frontier.md` before
spending team effort. See `docs/nofuncmap_classification.md` for what it is.
...
| Matching-C functions | `scripts/calcprogress.py` | funcs whose bytes come from compiling `src/*.c` (+ populated libc/libgcc); see `docs/frontier.md` for the current remainder. |
| Extracted data | `scripts/calcprogress.py` | source-form is an allow-listed object-root metric that permits legitimate FLOOR binary inputs; strict C/PNG is reported separately. Neither means named `.incbin "baserom.gba"` is extraction. |
| Named symbols | `scripts/calcprogress.py` | literal `.global` assembly labels without `sub_`/`data_`/`nullsub_` placeholders. This is not every ELF/C symbol and historical denominators may use a different census. |
