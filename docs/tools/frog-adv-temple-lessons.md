...
| 6 | `tools/agent/classify_unmatchable.py` + the NAKED-gate from `tools/agent/prompts/decomp.md` | Conservative unmatchability triage + honest defer/NAKED+`NON_MATCHING` fallback that keeps the build green. | Low / **Med** — niche (we relink mostly) but valuable for the unmatched region-different TUs (see `docs/frontier.md` for current count; the ~11 TU figure is a historical snapshot). |
