...
region-different candidate population in this classification cache.
...
additionally captures exact anchors for the classification cache.

ANALYSIS ONLY: never edits the build, never `git add`. Outputs:
  docs/nofuncmap_classification.md   -- the sized breakdown (counts + %)
  layout/nofuncmap_region_same.tsv   -- broad region-same classification cache
...
#     for the classification cache -- a guaranteed-correct subset, cheap to compute.
# --------------------------------------------------------------------------- #
_us_rom = None
_jp = None
...
def main():
    ...
    region_diff_rows = []  # (us, size, name, tu) -- env-gated classification cache
    sample_names = set()
    ...
    # Env-gated REGION-DIFFERENT classification cache (D54/D60).
    ...
    if os.environ.get("CLASSIFY_EMIT_RD"):
        ...
        with open(rd_out, "w") as f:
            f.write("# REGION-DIFFERENT no-funcmap US-C-portable classification cache.\n")
            f.write("# Generated by scripts/classify_nofuncmap.py with CLASSIFY_EMIT_RD=1.\n")
            f.write("# Broad candidate set; entries may already be carved. Do not dispatch\n")
            f.write("# from this cache; validate against docs/frontier.md and current layout.\n")
    ...
    # Add raw-exact anchors not already in the sampled region-same cache rows.
    ...
    # ----- classification-cache tsv -----
    os.makedirs("layout", exist_ok=True)
    with open(OUT_TSV, "w") as f:
        f.write("# Broad REGION-SAME no-funcmap US-C-portable classification CACHE.\n")
        f.write("# Generated by scripts/classify_nofuncmap.py (relocation-aware\n")
        f.write("# find_runs-style match + raw-byte exact pre-pass). Entries may already\n")
        f.write("# be carved. Do not dispatch: check docs/frontier.md and current layout.\n")
    ...
    A("")
    A("> **CLASSIFICATION CACHE, NOT THE WORK FRONTIER.** This is a broad,")
    A("> potentially inflated candidate cache; entries may already be carved.")
    A("> Do not dispatch agents from this report or its TSV. `docs/frontier.md` is")
    A("> the sole authoritative frontier and must be spot-checked against current")
    A("> layout before starting work.")
    A("")
    ...
    A("a sound LOWER bound and region-different a sound UPPER bound within this broad")
    ...
    A("")
    A("## Classification-cache statistics (relocation-aware)")
    A("")
    ...
    A(f"| **region-same (total)** | **{rs}** | **{pctn(rs):.1f}%** | **{pctd(rs):.1f}%** | **cached region-same candidates** |")
    A(f"| region-different | {rd} | {pctn(rd):.1f}% | {pctd(rd):.1f}% | cached zero-JP-match candidates |")
    ...
    if sampled:
        ...
        A("")
        A(f"- **region-same  ~= {est_rs:.0f}** ({pctd(rs):.1f}%) -- cached candidates")
        A(f"  for relocation-aware comparison; not a current dispatch list.")
        A(f"- **region-different ~= {est_rd:.0f}** ({pctd(rd):.1f}%) -- cached")
        A(f"  zero-JP-match candidates, not the authoritative current frontier.")
        rs_for_ceiling = est_rs
        rd_for_ceiling = est_rd
    else:
        A("### Full-pool result (exact) + inconclusive projection")
        A("")
        A(f"- **region-same = {rs} CONFIRMED** ({pctd(rs):.1f}% of decided) within")
        A(f"  this cache; these are candidates, not a current dispatch list.")
        A(f"  Every one is listed in `{OUT_TSV}`.")
        A(f"- **region-different = {rd} CONFIRMED** ({pctd(rd):.1f}% of decided) within")
        A(f"  this cache; this is not the authoritative current frontier.")
        ...
        rd_for_ceiling = est_rd
    A("")
    A("Historical classification implication for the source ceiling: combined with the")
    ...
    A(f"the ceiling), this cache estimates ~{rd_for_ceiling:.0f} no-funcmap functions")
    A("in the region-different class. It does not establish current remaining work.")
    A("")
    ...
    A("> waves describe. Some unique-masked-match functions may need their TU-private")
    A("> data bound first (e.g. the D45/D51 UnpackRaw class); others may already be")
    A("> carved. Consult the authoritative frontier and current layout.")
    A("")
    A("## Cached region-same candidates by TU (top 40)")
    A("")
    A(f"Descriptive distribution of cached region-same no-funcmap candidates")
    A(f"(`{OUT_TSV}` has {len(region_same_rows)} reloc-confirmed rows"
      + (f" + {len(exact_jp)} raw-exact anchors" if exact_jp else "")
      + "). Validate every entry against `docs/frontier.md` and current layout.")
    A("")
    A("| US TU | cached region-same no-funcmap candidates |")
    A("|-------|--------------------------------------:|")
    ...
    print()
    print("== NO-FUNCMAP CLASSIFICATION CACHE (reloc-aware) ==")
    print(f"  pool                    : {pool_n}")
    ...
    print(f"  region-same TOTAL       : {rs} ({pctd(rs):.1f}% of decided)  <- cached candidates")
    print(f"  region-different        : {rd} ({pctd(rd):.1f}% of decided)  <- cached candidates")
