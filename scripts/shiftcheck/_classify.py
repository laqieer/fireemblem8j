...
# fe8j (JP) divergence from fe8u: fe8u pins an absolute banim block at 0x08C00000 with slack
# below it, and silences shiftability suspects whose LOCATION is >= that pin. fe8j's generated
# ldscript packs the ENTIRE 16 MB ROM contiguously (no pinned-block-with-slack), and has 24 real
# carved placements above 0x08C00000 (banim_data, data_banim, pads). Setting PIN = ROM_HI keeps
# the whole ROM in scope so that real fe8j data is not wrongly silenced as "pinned". See
# docs/decisions.md D313.
PIN = ROM_HI          # fe8j: no absolute-pinned block -> nothing is silenced as "pinned"
