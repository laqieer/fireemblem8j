...
NOT APPLICABLE TO fe8j AS-IS (non-gating target): fe8j's generated ldscript packs
the entire 16 MB ROM contiguously (no slack) and uses a tail of absolute NOLOAD
overlay sections at fixed addresses; a uniform shift overflows the cart and desyncs
those overlays, so this layer cannot build a valid shifted ROM. The gating layers
are scan_build_addrs.py + scan_relocs.py + scan_offsets.py. See README.md / D313.
"""
...
ANCHOR_OBJ = "rom_header.o"  # fe8j: first .text object (no crt0). fe8u uses crt0.o.


def map_bounds(map_path):
    """Return (shift_start_vma, data_end_vma): anchor .text end, and last natural data.

    fe8j has no crt0.o; the first text object is src/rom_header.o. NOTE: even with a
    valid anchor, fe8j's ROM is packed to the full 16 MB (data_end == ROM ceiling),
    so a uniform shift overflows the cart -- Layer 2 is not applicable to fe8j and
    this target is non-gating (see README / D313).
    """
    ...
    for ln in open(map_path, errors="replace"):
        ...
        s = int(m.group(3), 16)
        obj = m.group(4)
        if obj.endswith(ANCHOR_OBJ) and m.group(1) == "text":
            anchor_end = a + s
        if s and ROM_BASE <= a < PIN:
            data_end = max(data_end, a + s)
    if anchor_end is None:
        sys.exit(f"could not find {ANCHOR_OBJ}(.text) in map (shift anchor)")
    return anchor_end, data_end
...
def main():
    ...
    print(f"shift_start (after {ANCHOR_OBJ}) = {shift_start:#010x} | "
          f"natural data end = {data_end:#010x} | ceiling = {PIN:#010x}")
