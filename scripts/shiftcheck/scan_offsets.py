...
def parse_syms(elf, nm):
    ...
    unavoidable coincidence of ROM layout. See docs/decisions.md D384.
    """
...
def main():
    ...
    for off, sv, nm in parse_relocs(args.elf, args.prefix + "readelf"):
        ...
        # the real source symbol, so a computed cross-resource "addend" is an
        # artifact of that collapse (must be caught at the source level instead --
        # see scan_raw_casts.sh). fe8u's sections are named ROM/IWRAM (in
        # SECTION_SYMS); fe8j names them .rom and .bss_<N> -- ALL section symbols
        # there start with ".", and no real fe8j global does (they are g*/PascalCase/
        # sub_*/data_*/song*). So skip SECTION_SYMS, anything nm flagged as a section,
        # and any "."-prefixed base. See docs/decisions.md D313.
        if nm in section_names or nm in SECTION_SYMS or nm.startswith("."):
            continue
        ...
        # ROM-layout luck, not a wrong-base bug. See docs/decisions.md D384.
