...
static s32 impl_fn(Side *S, struct SplineCtrlPoint *pts, s32 count, u32b t, struct Vec2s16 *out, u8 loop)
{
    ...
    /* MUTATION (harness_mut.c): swap the x/y output-copy words. This is a
     * behavior-changing "output copy" mutation (task requirement #7): it must
     * REFUTE the out->x/out->y equality assertions in main() whenever the
     * oracle's two shared result words differ, which is reachable throughout
     * the bounded domain (the oracle result is unconstrained nondet). */
    out->x = (s16)call_out[1];
    out->y = (s16)call_out[0];
    return i;
}
