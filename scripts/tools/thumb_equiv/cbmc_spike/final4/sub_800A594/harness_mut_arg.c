...
static s32 impl_fn(Side *S, struct SplineCtrlPoint *pts, s32 count, u32b t, struct Vec2s16 *out, u8 loop)
{
    ...
    call_out = outp;
    /* MUTATION (harness_mut_arg.c): corrupt the "count" logical argument
     * passed to the sub_800A34C shared oracle (literal 3 -> 2), a
     * behavior-changing "meaningful callee argument" mutation (task
     * requirement #7). This must REFUTE at the oracle's own
     * `g_a34c_count[k] == count` anti-masking assertion -- proving the
     * shared-oracle argument check, not just the final return/out compare,
     * is load-bearing. */
    o_sub_800A34C(S, lpts, call_out, ltimesp, t, 2);
