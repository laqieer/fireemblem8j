 ...
 */
/* MUTATION (requirement 8: alter availability/compaction/random-selection/
 * result-materialization and REFUTE through an observable assertion):
 * impl_fn's post-processing of the random pick drops the "- 1" that both
 * asm/sub_807D3BC.s (`subs r0, #1` right after `bl DivRem`) and every
 * independent reading agree on, i.e. `pick = DivRem(rn, count + 1);` instead
 * of `pick = DivRem(rn, count + 1) - 1;`. This directly changes WHICH
 * accepted candidate slot gets materialized into *result (a
 * random-selection/result-materialization class bug), and is reachable any
 * time the free grid produces >=2 accepted cells with a shared RNG value
 * whose two candidate picks (with and without the "- 1") land on different,
 * distinctly-positioned slots -- which the unconstrained domain exercises
 * directly, no scenario narrowing needed. (An earlier compaction-condition
 * mutation attempt, `boolAvailable != 0` instead of `== 1`, was found to be
 * UNDETECTABLE: array[0..count-1] is a true invariant of this algorithm --
 * every slot below the final `count` was locked to boolAvailable == 1 at the
 * moment count advanced past it and is never revisited, so indices[i] == i
 * always and the `==1`/`!=0` compaction test is provably always true either
 * way. That invariant is documented in README.md; this mutation avoids it.)
 */
#define MUTATE_IMPL 1
#include "common.h"
...
#else
    pick = (s8) (o_DivRem(S, rn, count + 1)); /* MUTATED: dropped "- 1", see file header */
#endif
