/* MUTATION of harness.c -- REQUIRED to REFUTE (proves the harness is
 * non-vacuous / actually sensitive to a real behaviour change, not just
 * trivially "PROVEN" by construction).
 *
 * THE MUTATION: impl_step's PutSpriteExt attribute-word constant is changed
 * from the real ASM's `+ 0x800` to `+ 0x400` (a plausible-looking but WRONG
 * OBJ-priority/palette-bank bit, i.e. exactly the class of one-bit-off
 * transcription bug this proof exists to catch). This changes the FIFTH
 * PutSpriteExt argument (`a5`, "attr" in the CallRec) on impl's side only;
 * ref_step (the m2c/asm-faithful side) is untouched. Expected verdict:
 * VERIFICATION FAILED, caught by
 * "cutpoint step: same call arg a5 (attr)" -- the exact assertion that
 * exists to police PutSpriteExt's attribute word (task requirement 6: compare
 * every call's layer/x/y, sprite-data identity/content, and attributes).
 *
 * ---- ORIGINAL FILE HEADER (harness.c) FOLLOWS, UNCHANGED, FOR CONTEXT ----
 *
 ...
 * audit in README.md; both fixes are proven load-bearing by
                       ...
                       int k, u8 nodeState, u8 iconPre, u8 iconPost, s16 nodeX, s16 nodeY)
{
    ...
    if (o_getpos(T, screenProcId, x1, y1, &xOut, &yOut))
    {
        ...
        /* MUTATED: real ASM/ref_step add 0x800 here; this is deliberately
         * wrong (0x400) so the harness must REFUTE -- see file header. */
        attr = (chrDiv + icon[iconIdx].sheetTileId) + ((pal & 0xF) << 12) + 0x400;
