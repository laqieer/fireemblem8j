/* MUTATION of harness.c -- BONUS: proves m2c CLEANUP FIX #1 (see file header)
 * is load-bearing, i.e. keeping it as `u8` (matching the RAW, uncleaned m2c
 * dump verbatim -- see m2c_ref_raw.c) rather than widening to `u16` is a REAL
 * behaviour change, not a cosmetic type-annotation choice.
 *
 * THE MUTATION: merge_ref's `var_r6` is reverted to raw m2c's literal `u8`
 * type. `var_r6 |= 0x400` on a genuine 8-bit C object truncates to 0 (0x400
 * has no bits in the low byte), silently dropping the OBJ-priority-ish flag
 * bit that AP_Update's y-argument must carry when
 * `proc->unk_34[...] & (1<<(nodeId&0x1F))` is set. Expected verdict:
 * VERIFICATION FAILED whenever that bit is set, caught by
 * "merge-phase: same y" (the AP_Update y-argument comparison) -- this is a
 * genuine PutSpriteExt/AP_Update-argument mutation per task requirement 10
 * (AP_Update's y argument, not PutSpriteExt this time).
 *
                       ...
                       int nodeId, u8 nodeState, u8 iconPre, u8 iconPost, s16 nodeX, s16 nodeY, s16 screenX, s16 screenY)
{
    ...
    {
        /* MUTATED: raw m2c's literal `u8 var_r6;` (cleanup fix #1 REVERTED --
         * see file header). `var_r6 |= 0x400` on a u8 truncates to 0. */
        u8 var_r6 = (u8) tmp0_2;
