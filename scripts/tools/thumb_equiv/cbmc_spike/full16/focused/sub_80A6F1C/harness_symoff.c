/* MODELING-INCOMPLETE (does NOT pass): keeping the payload-window base offset fully
 * symbolic makes `payload + off` trip CBMC --pointer-check / loop-unwind bounds in a
 * cascade (the equivalence asserts only fail AFTER the out-of-bounds pointer failure
 * corrupts the explored paths) -- a harness-modeling limitation of a symbolic base
 * pointer, NOT a real inequivalence. The SOUND, PASSING proof is harness.c (off pinned
 * to 0; the offset is applied identically on both sides, so off=0 is representative). */
...
static int impl_fn(Side *S)
{
    ...
    if (S->header[1] == (u16)((o_C20(S, arg, S->header[3]) + a) & 0x3FF) &&
...
int main(void)
{
    ...
    /* STRENGTHENING variant: gUnk_02014EF4 payload-window offset kept fully symbolic. */
    init_off = nondet_int();

    base3 = (int)(u16)g_d34_header[3];
    ASSUME(base3 >= 0);
    ASSUME(init_off >= 0);
    ASSUME(init_off + base3 <= PSIZE);
