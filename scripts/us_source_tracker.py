...
def calcprogress_axis2():
    """Return axis-2 values from calcprogress's canonical implementation.

    calcprogress.py currently exposes its computed metrics as module globals
    rather than a callable API. Execute that implementation directly and consume
    those values instead of duplicating its census or parsing its prose output.
    """
    with contextlib.redirect_stdout(io.StringIO()):
        values = runpy.run_path(os.path.join(ROOT, "scripts", "calcprogress.py"))
    keys = ("funcs", "func_lib", "func_asm", "func_done", "func_total")
    missing = [key for key in keys if key not in values]
    if missing:
        raise RuntimeError(
            "calcprogress.py did not expose axis-2 values: " + ", ".join(missing))
...
def main():
    ...
    L.append("")
    L.append("This tracker and `scripts/calcprogress.py` intentionally measure different")
    L.append("universes. The inventory above classifies reference-US map symbols for")
    L.append("port planning. Calcprogress axis 2 counts the current linked JP build:")
    L.append(f"**{axis2['func_done']}/{axis2['func_total']} functions done** = "
             f"{axis2['funcs']} compiled from project `src/*.c` + "
             f"{axis2['func_lib']} linked from libc/libgcc, with "
             f"{axis2['func_asm']} still descriptive assembly.")
    L.append("These populations and percentages are not directly interchangeable.")
    L.append("")
