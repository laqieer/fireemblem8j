/* The JP build sign-extends the isBallista argument (asrs r2, #24) instead of
 * zero-extending it, so this TU must compile StartEventBattle with a signed s8
 * isBallista parameter. The shared functions.h prototype declares it u8 (and
 * every other TU relies on that). To match JP codegen without editing the
 * shared header, we hide that single prototype for this TU only: rename it via
 * a macro across the global.h include, then restore the real name for our own
 * s8 declaration and definition below. */
#define StartEventBattle StartEventBattle_u8_proto_hidden
#include "global.h"
#undef StartEventBattle
...
/* Local s8-isBallista prototype matching the JP signed-extension codegen. */
