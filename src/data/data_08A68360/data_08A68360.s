/* Migrated from asm/data_08A68360.s (region-same graphics, single section).
 * Each symbol kept in the original section/order; byte-identical via INCBIN_U*.
 */
	.section .data.residue.08A68360, "aw", %progbits
	.global EventScr_Ch14A_2
EventScr_Ch14A_2:
	.4byte 0x00020540
	.4byte frontier_df3_unitdef_b_001_91020C + 0x35C
	.4byte 0x00000A40
	.4byte data_08A60354 + 0x58
	.4byte 0x00070228
	.4byte 0x00000120
/* de-pointered slice EventScr_Ch14A_2: ptr=2 data=4 skip=0 */
	.section .data.residue.08A68378, "aw", %progbits
	.global EventScr_Ch14A_3
EventScr_Ch14A_3:
	.4byte 0x00020540
	.4byte UnitDef_Ch14AEnemy_4
	.4byte 0x00000A40
	.4byte data_08A60354 + 0x58
	.4byte 0x00070228
	.4byte 0x00000120
/* de-pointered slice EventScr_Ch14A_3: ptr=2 data=4 skip=0 */
	.section .data.residue.08A68390, "aw", %progbits
	.global EventScr_Ch14A_4
EventScr_Ch14A_4:
	.4byte 0x00020540
	.4byte 0x00000000
	.4byte 0x00000A40
	.4byte UnitDef_Ch14BAlly_7
	.4byte 0x000C0221
	.4byte 0x00070228
	.4byte 0x00000120
/* de-pointered slice EventScr_Ch14A_4: ptr=1 data=6 skip=0 */
	.section .data.residue.08A683AC, "aw", %progbits
	.global EventScr_Ch14A_5
EventScr_Ch14A_5:
	.4byte 0x00020540
	.4byte UnitDef_Ch14AEnemy_2
	.4byte 0x00000A40
	.4byte data_08A60354 + 0x58
	.4byte 0x00070228
	.4byte 0x00000120
/* de-pointered slice EventScr_Ch14A_5: ptr=2 data=4 skip=0 */
	.section .data.residue.08A683C4, "aw", %progbits
	.global EventScr_Ch14A_6
EventScr_Ch14A_6:
	.4byte 0x00020540
	.4byte 0x00000000
	.4byte 0x00000A40
	.4byte UnitDef_Ch14BAlly_7
	.4byte 0x02000F21
	.4byte 0x000E0221
	.4byte 0x00070228
	.4byte 0x00000120
/* de-pointered slice EventScr_Ch14A_6: ptr=1 data=7 skip=0 */
	.section .data.residue.08A683E4, "aw", %progbits
	.global EventScr_Ch14A_7
EventScr_Ch14A_7:
	.4byte 0x00020540
	.4byte frontier_df3_unitdef_b_002_9105E0
	.4byte 0x00000A40
	.4byte data_08A60354 + 0x58
	.4byte 0x00000F23
	.4byte 0x000E0221
	.4byte 0x00000F20
	.4byte 0x00000C41
	.4byte 0x0000000C
	.4byte 0x000E0229
	.4byte 0x00000820
	.4byte 0x00070228
	.4byte 0x00000120
/* de-pointered slice EventScr_Ch14A_7: ptr=2 data=11 skip=0 */
