...
/* Battle/combat terrain lookup tables (region-same prefix of fe8u
 * src/data_terrains.c), JP 0x0885FD60..0x088617C8 (0x1A68 bytes, .data):
 * per-class movement-cost tables (Normal/Rain/Snow), avoid/def/res terrain
 * bonuses, heal amount + heals-status, and the battle-anim terrain ground /
 * BG LUT tables. These bytes are byte-identical to the recompiled US source.
 * The fe8u tail (Unk_TerrainTable_8..11 + gTerrains_0 tile-id table) is
 * region-DIFFERENT in the JP ROM and stays as raw data, so this TU stops
 * right after gBanimBGLut14. */
