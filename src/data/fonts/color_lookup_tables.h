...
/* Storage-class/section prefix for each LUT. fe8u places these in the
 * fontgrp-data object's default .data; the JP build pins them to a named
 * subsection (.rodata.dat_fontgrp_data) for byte-identical linker placement,
 * so the includer may override FONTGRP_LUT before including this file. */
#ifndef FONTGRP_LUT
#define FONTGRP_LUT
#endif
...
#define COLOR_CONVERT(pixel) ((pixel == 0) ?  4 : \
                              ...
                              (pixel == 3) ? 15 : \
                              0)
FONTGRP_LUT u16 gFontgrp_11[] = { GENERATE_LOOKUP_TABLE };
#undef COLOR_CONVERT

#define COLOR_CONVERT(pixel) ((pixel == 0) ? 15 : 0)
FONTGRP_LUT u16 gFontgrp_12[] = { GENERATE_LOOKUP_TABLE };
#undef COLOR_CONVERT

#define COLOR_CONVERT(pixel) (pixel + 4)
FONTGRP_LUT u16 gFontgrp_13[] = { GENERATE_LOOKUP_TABLE };
#undef COLOR_CONVERT
...
#define COLOR_CONVERT(pixel) ((pixel == 0) ? 4 : (pixel + 12))
FONTGRP_LUT u16 gFontgrp_14[] = { GENERATE_LOOKUP_TABLE };
#undef COLOR_CONVERT

#define COLOR_CONVERT(pixel) ((pixel == 0) ? 4 : (pixel + 4))
FONTGRP_LUT u16 gFontgrp_15[] = { GENERATE_LOOKUP_TABLE };
#undef COLOR_CONVERT
