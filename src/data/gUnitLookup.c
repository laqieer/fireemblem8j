...
/* gUnitLookup: maps a unit id (faction byte | slot index) to the EWRAM Unit
 * struct for that slot. Region-same structure as fe8u (src/bmunit.c); the only
 * JP difference is the RAM addresses of the four faction arrays, which the
 * linker resolves from layout/baseline_syms (extern absolute symbols). */
